/**
 * Per-element error boundary (plan.md §11.4; T29A1).
 *
 ...
 * must never take down the whole transcript/rail. React error boundaries
 * can only be class components, so this one file stays a class rather than
 * following the rest of this app's function-component convention.
 */
import { Component, type ErrorInfo, type ReactNode } from "react";

import type { Logger } from "@picompanion/frontend-core";

import { ErrorState } from "../../ui/primitives/index.js";
 ...
 * shared `ErrorState` primitive (`role="alert"`, plan.md §10.5) in place of
 * just that element; every sibling element keeps rendering normally.
 */
...
> {
  ...
  override componentDidUpdate(prevProps: ExtensionElementBoundaryProps): void {
    ...
    const identityChanged =
      prevProps.ns !== this.props.ns || prevProps.elementId !== this.props.elementId;
    const resetKeyChanged = prevProps.resetKey !== this.props.resetKey;
    if (identityChanged || resetKeyChanged) {
