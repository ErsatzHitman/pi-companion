 ...
 * with undo"; §11.7 `prompt-arbitrage`) — the pure decision of what an
 * accepted or undone proposal does to the live composer draft.
 *
 * The proposal card itself is the `extensions` feature's renderer
 * (`features/extensions/renderers/composer.tsx`), and its `accept`/`undo`
 * actions already dispatch through the shared
 * `ExtensionActionController`. This module is the other half: it turns a
 * *settled* accept into a draft write — `payload.text` under
 ...
 * when the daemon supplied none), so a user can accept a rewrite and
 * change their mind.
 *
 * It deliberately owns no React and no transport. The subscription seam
 * is `PiUiComposerDraftSource` below — a narrow, structurally typed shape
 * any `ExtensionActionController` already satisfies — so `features/
 * composer` never imports `features/extensions` (the sibling-feature
 * rule: only routes compose across features). The route-chrome that
 * mounts both the composer and the Pi UI rail builds the source and
 * passes it down, exactly as it already passes `readClient`/`writeClient`
 * and every `resolve*Client` port.
 *
 * Two deliberate rules:
 *
 * - **A blank proposal never clears the draft.** `payload.text` that is
 *   empty or whitespace-only is a no-op, so an extension bug cannot
 *   silently discard what the user typed (the same guarantee
 *   `../voice/voice-model.ts`'s `applyTranscriptToDraft` makes for a
 *   blank transcript).
 * - **Only `accept` and `undo` settle into a draft write.** Any other
 *   settled action on a composer element — a `dismiss`/`decline` action,
 *   or a failure/timeout — leaves the draft exactly as it was.
 */
 ...
 * composer-kind proposal is resolved. In production the route-chrome
 * builds this over the same `ExtensionActionController` and
 * `PiUiElementStore` the rail already uses; a test supplies a fake.
 */
export interface PiUiComposerDraftSource {
  /** Subscribes to every pending/settled action transition for this agent. */
  subscribe(
    listener: (target: PiUiComposerActionTarget, state: PiUiComposerActionState) => void,
  ): () => void;
  /**
   * The composer-kind proposal this action target belongs to, or `null`
   * when the target is not a composer element (or the element is gone).
   */
  resolveProposal(target: PiUiComposerActionTarget): PiUiComposerProposal | null;
}

/**
 * The narrow slice of a live Pi UI session (`features/extensions`'
 * `PiUiSessionValue`) this source needs. Structurally typed rather than
 * importing that feature's type, so `features/composer` stays free of
 * sibling-feature imports; the route-chrome passes the real session's
...
export interface PiUiComposerDraftSession {
  ...
  agentId: string;
  /** The live action controller's own `subscribe` (every pending/settled transition). */
  subscribe(
    listener: (target: PiUiComposerActionTarget, state: PiUiComposerActionState) => void,
  ): () => void;
  /** The live element store's own `getElement`. */
  getElement(agentId: string, namespace: string, elementId: string): PiUiElement | undefined;
}

/**
 * Builds the `PiUiComposerDraftSource` a route-chrome passes to the
 * composer from its live Pi UI session. Kept here (not in the route) so the
 * "is this target a composer element carrying a valid payload" decision is
 * unit-tested once rather than re-derived at each mount; a route supplies
 * only the session's own stable `subscribe`/`getElement`.
 */
 ...
 * rule `../voice/voice-model.ts` uses for a transcript, reproduced here
 * because the two live in different workspaces (the same two-consumer,
 * small-module duplication `editor-text-model.ts`'s header documents).
 */
 ...
 * and `prefill` both land the proposed text as the draft; `append` adds
 * it to whatever is already there.
 */
 ...
 * argument, so a caller can compare and a test can assert without hidden
 * state.
 */
export function resolvePiUiComposerSettlement(
  draft: string,
  ...
  if (target.actionId === PI_UI_COMPOSER_ACCEPT_ACTION_ID) {
    ...
    // had"; fall back to the draft this client held at accept time when
