 ...
 * every `setTimeout` whose due time has arrived, in due order, so the
 * composer's debounced draft writer (`DraftSessionController`) can be
 * tested without a real timer. `setInterval` is unimplemented.
 */
