/**
 * The three rewind scopes `agent.rewind.request` accepts, with the one
 * honest sentence each carries in the dialog — T395, `plan.md` §4.2
 ...
 * the dialog from inventing one. The descriptions are deliberately
 * literal about what each scope touches, because the three restores are
 * not interchangeable: a conversation rewind navigates Pi's session tree,
 * a files rewind restores the snapshot, and only `"both"` does the two
...
/** The three scopes, in the order the dialog offers them (chat, files, both). */
