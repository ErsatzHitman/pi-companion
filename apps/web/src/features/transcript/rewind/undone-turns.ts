 ...
 * snapshots").
 *
 * Each successful rewind this browser session performs is remembered
 * locally as the message it rewound *to*: the daemon exposes no list of
 * past rewinds (`agent.rewind.response` carries only `ok` and an
 * `error` string), so a screen that offered a "return to that turn"
 * affordance without recording it itself would be inventing daemon state
 * it does not have. This module is the honest local record: a bounded,
 * in-memory list of `{ messageId, snippet, mode }`, owned by the hook
 * (`use-rewind-to-here.ts`) and rendered by `RewindDialog.tsx` under an
 * explicit "local record, not daemon state" label.
 *
 * **Why the rewound-to turn, not the turns it removed.** The removed
 * turns' ids are no protection against a return: for a submitted prompt
 * the client only ever holds the `clientMessageId`, and
 * `agent-manager.rewind` maps that to the provider's tree entry through
 * the live timeline row (`packages/server/src/server/agent/agent-manager.ts`).
 * Once a rewind removes that row, the mapping is gone and a return would
 * call Pi's `navigateTree` with an id that is not a tree entry. The turn
 * a rewind targeted is retained, so its id always resolves, and
 * re-issuing a conversation rewind to it is exactly "return to that
 * turn".
 *
 * The record is deliberately **not** persisted: it is a record of actions
 * taken in this browser session, so a reload starts it empty rather than
 * pretending the list survived one. `MAX_UNDONE_TURNS` bounds it.
 *
 ...
 * single-character ellipsis when it truncates. Never returns a
 * multi-line string, so a list row is always one line.
 */
 ...
 * `(messageId, mode)` pair (a repeated rewind is one list row, not two)
 * and capping the list at `MAX_UNDONE_TURNS`.
 */
