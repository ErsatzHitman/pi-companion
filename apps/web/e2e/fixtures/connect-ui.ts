 ...
 *
 * CORRECTED (FIX-CI9): this helper used to end by asserting the
 * `connect-status-banner` reads `Reached <label>`, and its callers'
 * comments said "`connectViaUi` never navigates away". Both stopped being
 * true when a successful connect started landing the user on that host's
 * session list: `ConnectForm` sets that status message and calls
 * `onConnected` in the same tick, so the router leaves `/connect` before
 * the banner is ever painted and the assertion times out with
 * `element(s) not found`. Roughly thirty specs across nine files failed on
 * that one assertion. The destination IS the success signal now, so this
...
export async function connectViaUi(
  ...
  // The saved profile id is generated by `hosts.HostProfileStore`, not
  // chosen here, so the segment is matched rather than named. Waiting on
  // the URL also settles the navigation before a caller's own `page.goto`
  // runs, which would otherwise race it.
