...
describe("todo-row.tsx: the artifact's .ov widget (T360)", () => {
  ...
  it("prints a per-state glyph and a #n, so colour is never the only signal", () => {
    const code = readCode();
    expect(code).toMatch(/\{todoGlyph\(row\.state\)\}/);
    expect(code).toMatch(/`#\$\{row\.number\}`/);
    expect(code).toMatch(/\{row\.connector\}/);
  });
  ...
  it("hides the ring from assistive tech and announces the widget as a whole", () => {
    const code = readCode();
