...
describe("applyComposerProposal", () => {
  ...
  it("replaces the draft for prefill mode too", () => {
...
describe("applyPiUiComposerSettlement", () => {
  ...
  it("undo restores the recorded previous draft and clears the record", () => {
    const accepted = applyPiUiComposerSettlement(
      "typed",
      NO_RECORDS,
      target(PI_UI_COMPOSER_ACCEPT_ACTION_ID),
      settled(PI_UI_COMPOSER_ACCEPT_ACTION_ID),
    ...
    const undone = applyPiUiComposerSettlement(
      accepted!.draft,
  ...
  it("a failed or pending settlement is a no-op even on accept", () => {
    expect(
      applyPiUiComposerSettlement(
        "typed",
        NO_RECORDS,
  ...
  it("does not mutate the caller's previousByElement map", () => {
