/**
 * `markdown` kind render model (plan.md §11.3, §16 "sanitize markdown ...
 * do not render extension-provided HTML"; T34A3) — "Rich textual content".
 *
 * This module never touches any renderer; it turns the payload's `text`
 * into a plain AST that `markdown.tsx` maps onto `Text`/`View` elements —
 * never a `WebView` or any other HTML-interpreting surface. That is the
 * whole sanitization story on this platform: React Native has no
 * `dangerouslySetInnerHTML` equivalent to begin with, and a string like
 * `<img onerror=...>` embedded in the source text is never parsed as
 * markup here — it is carried through as a literal `text` node and
 * rendered as plain, inert characters inside a `Text` component.
 *
 * This is a fresh, RN-free module rather than an import of the web
 * renderer's `markdown-parse.ts` — cross-workspace source imports are not
 * allowed (plan.md §5), and web/Android deliberately keep separate
 * renderers over one shared meaning (plan.md §18.3). The subset parsed is
 * identical to the web renderer's, field for field
 * (`apps/web/src/features/extensions/renderers/markdown-parse.ts`):
 * headings, paragraphs, fenced code blocks, block quotes, ordered/
 * unordered lists, horizontal rules, and the common inline spans
 * (`**bold**`, `*em*`/`_em_`, `` `code` ``, `[text](href)`).
 */
...
/**
 * True for a relative-looking href (no `scheme:` prefix) or one using an
 * allow-listed scheme. `javascript:`, `data:`, `vbscript:`, and anything
 * else fall through to `false` — matching the web renderer's guard.
 */
  ...
  while (rest.length > 0) {
    ...
    if (match[1] !== undefined) {
      ...
      if (isSafeMarkdownHref(href)) {
        ...
        // Unsafe scheme: keep the literal source text visible, but never as
        // a navigable link.
...
/**
 * Flattens an inline node tree to its plain text content, dropping
 * formatting. Used for the `Link` primitive's `label` prop, which — unlike
 * the web `Link`'s `children` — only accepts a flat string.
 */
export function flattenInlineText(nodes: readonly MdInlineNode[]): string {
  return nodes
    .map((node) => {
      switch (node.type) {
        case "text":
...
export interface PiUiMarkdownRenderModel {
...
export function buildMarkdownRenderModel(
  ...
  payload: PiUiPayloadForKind<"markdown">,
): PiUiMarkdownRenderModel {
  const title = element.title;
  return {
    title,
    blocks: parseMarkdown(payload.text),
    actionsAccessibilityLabel: `${title ?? "Markdown"} actions`,
  };
}
