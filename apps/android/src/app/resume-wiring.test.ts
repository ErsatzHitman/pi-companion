...
// T32S11: `../app-shell/core.ts` now constructs `createExpoFilePicker`,
// whose module imports `expo-document-picker`/`expo-image-picker` at its top
// level (see that platform file's own header). Both are mocked the same way
// every other native module in this file is, so importing the real packages
// never drags their native/`react-native` graph through plain vitest. This
// file never calls the picker.
...
// T32S11: `../app-shell/core.ts` now constructs `createExpoSharing`, whose
// module imports `expo-sharing`/`expo-file-system/legacy` at its top level
// (see that platform file's own header). Both are mocked for the same
// reason above; this file never shares a file.
...
// T32S11: `../app-shell/core.ts` imports the `../features/terminal` barrel,
// which re-exports `TerminalScreen` -> `terminal-webview-host.tsx` -> the
// real `react-native-webview`. Stood in for the same reason every other
// native component above is; this file never renders.
