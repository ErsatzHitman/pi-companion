...
class FakeDaemonClient {
  ...
  private readonly piUiActionResponseHandlers = new Set<(message: unknown) => void>();
  on(type: "agent_stream", handler: (message: AgentStreamMessage) => void): () => void;
  on(type: "pi.ui.action.response", handler: (message: unknown) => void): () => void;
  on(
    type: "agent_stream" | "pi.ui.action.response",
    handler: ((message: AgentStreamMessage) => void) | ((message: unknown) => void),
  ): () => void {
    ...
    const responseHandler = handler as (message: unknown) => void;
