...
as T372 · **Refreshed again:** 2026-09-11 at `6c55c13` (T378), as T379 · **Refreshed again:**
2026-09-11 at `9f12d23` (T381 close-out) · **Refreshed again:** 2026-09-16 at `2c9c254`
(FIX-CI9), as DOCS-1 · **Branch:** `main` ·
...
**What this refresh changed (DOCS-1, 2026-09-16 at `2c9c254`).** Everything from the redesign
work this file documents (§4–§10) predates a whole later stretch of web work that this refresh
does not attempt to narrate in the same depth — it only orients the next reader to it and stops
§1 from being read as current. Since the last refresh, `main` gained: **UI-X11–UI-X14**, four
commits polishing `/connect` (`b5ed3ce` fixed the screen's own padding/form-rhythm/button-row
regressions, `f6ae832` moved each rail-collapse toggle to the side it controls, `83462c8` ported
Beautiful UI's `fade-up`/`pop-in` entrance motion onto the connect card and form rows, and
`b552da7` made a successful connect navigate to the host's session list instead of leaving the
user stranded on a static "Signed in to…" banner); **DEPLOY-1–DEPLOY-3**, which added a static-host
SPA fallback (`f111862`) and then iterated the hosting write-up twice — `d559eff`'s Cloudflare
Pages + Tunnel plan was superseded by `6ce71fc`'s record of what was actually stood up (GitHub
Pages for the UI, an ngrok tunnel to the daemon, bcrypt password enforcement on both REST and
WebSocket); and the **FIX-CI1–FIX-CI9** series repairing CI regressions the above (and the design
system's own TOKENS-1 work) introduced, ending in `2c9c254` (current HEAD), which fixed two
independent `web-tests` regressions — an axe `color-contrast` scan sampling mid-animation after
UI-X13, and a Playwright connect helper waiting on the banner UI-X14 made unreachable — and, in
the same commit, repaired `packages/design-tokens/src/tokens.ts`'s block comment that had made
the whole package unparseable, restored the `teal` colour role, and moved
`typography.letterSpacing.wide` to `0.09`. **CI run `35077563755` at this exact HEAD was still
`in_progress` at the moment this paragraph was written** — re-derive its conclusion with
`gh run view 35077563755 --json status,conclusion` before trusting anything about "green" below
this point; do not treat "the fix looks right" as equivalent to a read run. §1's `Fact | Value`
table and its "Wave 3" paragraph are demoted to historical for exactly this reason: both describe
HEAD as `aac2075`/`2cda7b7`, five and four refreshes behind the HEAD above.
...
**Historical (superseded 2026-09-16 by the DOCS-1 refresh above — this paragraph and the
`Fact | Value` table below it both describe a HEAD several refreshes old and are kept only as
the record of the T383–T396 wave, not as current state).**
...
| Fact                                                                            | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HEAD (historical — see the DOCS-1 refresh note above for the real current HEAD) | `aac2075` — "T387 follow-up: record the wave's two green gates"; the commit you are reading is one past it. The wave's product commits: `9afc612` (T384 + T385, Android chrome/Sessions/Live/chat), `bf47161` (T386, the web console), `aea1248` (T387, inline extension elements), `34509d5` and `d44fc0f` (the two T386 follow-ups); `c05016d` and `aac2075` carry prose only                                                                                                                                                                                                                                                                                                                                                                                                                     |
| CI (green)                                                                      | run `34647734287` at `aac2075` — **completed / success**, 44 jobs (42 success, 2 skipped by the path filters: `nix-checks`, `docker-checks`), zero failures: `changes` (the `scripts/ci` guard suite), `typecheck`, `lint`, every repo guard, `web-unit-tests` on ubuntu + windows, `web-tests` (Playwright), `server-tests` on ubuntu + windows, `android-tests`. Predecessor `34645663675` at `c05016d` was also completed/success. The one red run of the wave, `34644982421` at `d44fc0f`, failed `changes` alone: `guard-format-check-per-commit.test.mjs`'s real-git fixture asserts `git rev-parse --is-shallow-repository` is `false`, and the history rewrite left that runner fetching `--depth=1`, so the history it needs was genuinely absent. The same suite passes locally (895/895) |
| Maestro (green)                                                                 | run `34645136806` at `d44fc0f` — **completed / success**: `build-development-apk`, `shard-matrix`, `packaged-app-smoke`, `maestro-non-gating` and all five `maestro-e2e` shards (1, 2, 3, 4, 5). This is the on-device evidence for the Android half of the wave; the flows ran against the same `apps/android` tree the final commits carry. Predecessor `34592333116` at `9f12d23` was 9/9 green                                                                                                                                                                                                                                                                                                                                                                                                  |
| The ten §14.4 flows                                                             | all `PASS` at the last read dispatch (`34592333116`): `pairing`, `network-switch`, `cold-start-restore`, `background-kill-restore`, `composer-inputs`, `offline-cache-outbox`, `notification-approval`, `extension-sheets`, `files-and-terminal`, `accessibility-audit`. That is `plan.md`'s Phase 5 exit-gate criterion, met at that commit; the wave's own dispatch is the one to read now                                                                                                                                                                                                                                                                                                                                                                                                        |
| Working tree                                                                    | clean (`node scripts/ci/run-guard-clean-working-tree.mjs` → exit 0)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Ledger                                                                          | `docs/issues-from-plan.md` has rows and sections through T387 (596 tasks, counted from the table's own rows). T384–T387's sections record what each adopted from `docs/ui-reference/` and, more usefully, what each deliberately did NOT adopt and why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Local gate baseline                                                             | `oxfmt --check .` clean · `oxlint` 12 warnings, 0 errors (unchanged) · `node --test scripts/ci/*.test.mjs` 895/895 · `run-guard-capability-prose.mjs` exit 0 over 65 capability groups · android workspace suite 260 files / 3617 tests · web workspace suite 172 files / 1629 tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**What T384–T387 did, and the one decision the next agent needs from it.** The owner's
`docs/ui-reference/` pages were audited frame by frame against both apps by six read-only
agents, and the result is now four ledger sections plus a project-memory entry. The short
version: **the audit separates VISUAL fidelity from STRUCTURAL conflict.** Colour, radii, type
sizes, spacing, borders, iconography and element order were adopted wherever they did not break
a hard rule. Three kinds of finding were NOT adopted, each with its reason recorded: the
reference's smaller boxes were overruled by plan.md §9.3's 48dp touch floor; Settings' four
per-agent rows and its extension list stay out per T366's argued decision; and `docs/ui-reference/
pi-companion-app.html`'s five **extension frames are drawings, not routes** — `plan.md` §9.2 now
says so, the mockup README says so, and the rendering they describe (inline transcript blocks,
`[ns]` tags, aligned key/value lines, tone glyphs, the docked todo widget, the `Needs you` pill)
is what shipped. Do not file five extension-detail screens against those frames.

The reference files also carried a real defect, repaired here: `pi-companion-app.html`'s
`paintCtx()` assigned `.className` on an `<svg>`, which is read-only, so the script threw at load
and the README's "plays one whole turn end to end" was false in every browser.

**One history repair, recorded because the guard will otherwise look unexplained.** The wave's
first push (`2113e4b`) went red on a single job, `guard / format:check per commit`, naming
`4fbfa6e` (T387) as the commit that turned `docs/issues-from-plan.md` format-red — a ledger
section written through a heredoc and committed before `oxfmt` saw it. That guard compares each
commit against its own parent and never reads a later commit, so the reformatting commit that
followed could not clear it; the fix was the one the guard names, `git rebase -i` onto `9344932`
to amend that commit's own ledger blob, then a force-push. `4fbfa6e` is now `aea1248` and the
two follow-ups are `34509d5`/`891d6d5`; the dropped `2113e4b` had become empty. Tree content is
otherwise identical — `git diff backup/wave2-preformat..HEAD` is empty.
