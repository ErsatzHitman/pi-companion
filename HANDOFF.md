...
| CI for `d44fc0f`                 | read it with `gh run list --branch main --limit 3` before trusting this row. That run's `changes` job went red on `guard-format-check-per-commit.test.mjs`'s real-git fixture, which asserts `git rev-parse --is-shallow-repository` is `false`: the history rewrite recorded below left that runner fetching `--depth=1`, so the fixture's history was genuinely absent from that checkout. The same suite passes on this tree locally (`node --test scripts/ci/*.test.mjs`, 895/895), and the next push's run is the one to read. Prior green: `34596653634` at `9344932` |
| Last Maestro dispatch            | run `34645136806`, dispatched for this wave's Android changes (chrome, Sessions, Live, chat, extension rendering) at `d44fc0f`. Its predecessor `34592333116` at `9f12d23` was 9/9 green including `maestro-non-gating`                                                                                                                                                                                                                                                                                                                                                     |
