...
| Fact                             | Value                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HEAD                             | `be00baa` — "Fix format-only CI red on 9f12d23; refresh handoff through the T381 close-out", pushed to `origin/main`. T380 (`d11a6cb`), T381 (`a94012d`), T382 (`c49bf0e`) all landed since the last refresh                                                                                                                                                                                                               |
| CI for `be00baa`                 | run `34595072950`, success. Prior: `34592330251` for `9f12d23` red on `format` only (T381 ledger list indentation), fixed here                                                                                                                                                                                                                                                                                             |
| Last Maestro dispatch            | run `34592333116` at `9f12d23`, **9/9 green including `maestro-non-gating`** (`file-download: PASS`, `queue-retry-compaction: PASS`). Before it: `34589238596` at `a94012d`, same green picture. The ten §14.4 flows all `PASS` in both; `shards.json` untouched. No dispatch on `be00baa` — prose-only (ledger, handoff, flow comments), no screen a flow drives                                                          |
