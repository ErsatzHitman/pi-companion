...
same way. One row below is deliberately NOT in the flow (`composer-footer-ctx`), and declares
...
| Composer — context readout                           | `composer-footer-ctx`                                       | Swipe to the readout at the right end of the pill row above the prompt bar  | Announced as a sentence naming the fill — "Context. 34% of 200,000 tokens used." from `features/composer/context-pill-model.ts`'s `buildContextPillViewModel`, or "Context usage unknown…" before any `usage_updated` arrives. No hint, and no "double tap to": this is a readout, not a button — the design artifact gives `.fp.f-ctx` `cursor:default` and no press state, and the three controls beside it (mode, model, effort) are the interactive ones. Never the bare "34%" it draws: the pill is a single `accessible` View, so its percent, token count and bar merge into that one sentence. **Not sampled mechanically** — the audit flow does not reach this control, so this row is its only coverage |
