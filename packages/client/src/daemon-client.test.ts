...
import { BROWSER_AUTOMATION_COMMAND_NAMES } from "@picompanion/protocol/browser-automation/rpc-schemas";
import {
...
} from "@picompanion/protocol/terminal-stream-protocol";
...
test("cancelUpload sends the upload's requestId and does not resolve cancelled until the daemon confirms it", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const resultPromise = client.cancelUpload("req-upload");
  ...
  const sent = z
    ...
    .parse(JSON.parse(assertStr(mock.sent[0])));
  expect(sent.message.uploadRequestId).toBe("req-upload");
  // The cancel call's own correlation id must not collide with the
  // upload's requestId — they identify two different in-flight requests.
  expect(sent.message.requestId).not.toBe("req-upload");

  mock.triggerMessage(
    wrapSessionMessage({
      type: "file.upload.cancel.response",
      payload: {
...
test("cancelUpload reports cancelled: false when the daemon has nothing pending for the id", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const resultPromise = client.cancelUpload("req-already-done");
  const sent = z
    .object({
      type: z.literal("session"),
      message: z.object({ type: z.literal("file.upload.cancel.request"), requestId: z.string() }),
    })
    .parse(JSON.parse(assertStr(mock.sent[0])));
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        requestId: sent.message.requestId,
        uploadRequestId: "req-already-done",
        cancelled: false,
        error: "No upload in progress for this id.",
      },
    }),
  );
...
// `summary`/`estimatedTokensAfter`/`filesRead`/`filesModified`) actually
// reaches a `DaemonClient` subscriber intact — the "protocol -> client" hop
// the schema-only test in `packages/protocol` cannot prove by itself,
// since `z.object` silently drops unrecognized keys rather than failing.
test("parses canonical agent_stream compaction payloads carrying Pi's full result (T143)", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        agentId: "agent_pi",
        timestamp: "2026-09-05T20:20:00.000Z",
        event: {
          type: "timeline",
          provider: "pi",
          item: {
...
test("openTerminalSession: writeInput sends an actually-received encoded frame over the wire, not just a registered call", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const openPromise = client.openTerminalSession("term-t62");
  const subscribeRequest = z
    .object({ type: z.literal("subscribe_terminal_request"), requestId: z.string() })
  ...
  const outcome = session.writeInput(new TextEncoder().encode("ls\r"));
  expect(outcome).toBe("sent");

  expect(mock.sent).toHaveLength(1);
  const frame = decodeTerminalStreamFrame(assertUint8Array(mock.sent[0]));
  expect(frame?.opcode).toBe(TerminalStreamOpcode.Input);
  expect(frame?.slot).toBe(21);
  expect(new TextDecoder().decode(frame?.payload ?? new Uint8Array())).toBe("ls\r");
});

test("openTerminalSession: onOutput actually receives a daemon-delivered output frame for this terminal", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const openPromise = client.openTerminalSession("term-t62b");
  const subscribeRequest = z
    .object({ type: z.literal("subscribe_terminal_request"), requestId: z.string() })
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  mock.triggerMessage(
  ...
  session.onOutput((data) => received.push(new TextDecoder().decode(data)));

  mock.triggerMessage(
    encodeTerminalStreamFrame({
      opcode: TerminalStreamOpcode.Output,
      slot: 22,
      payload: new TextEncoder().encode("total 0\n"),
    }),
  );
...
test("openTerminalSession: close() unsubscribes, and a write after close is dropped-closed without reaching the wire", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const openPromise = client.openTerminalSession("term-t62c");
  const subscribeRequest = z
    .object({ type: z.literal("subscribe_terminal_request"), requestId: z.string() })
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  mock.triggerMessage(
  ...
  session.close();
  const unsubscribeMessage = parseSentFrame(mock.sent[mock.sent.length - 1]);
  expect(unsubscribeMessage.type).toBe("unsubscribe_terminal_request");

  mock.sent.length = 0;
  const outcome = session.writeInput("too late");
  expect(outcome).toBe("dropped-closed");
  expect(mock.sent).toHaveLength(0);
});

test("openTerminalSession: a frame for a never-subscribed slot fires onTerminalFrameDropped, not an exception", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  client.onTerminalFrameDropped((reason, slot) => drops.push({ reason, slot }));
  ...
  expect(() =>
    mock.triggerMessage(
      encodeTerminalStreamFrame({
        opcode: TerminalStreamOpcode.Output,
        slot: 99,
        payload: new TextEncoder().encode("nobody subscribed to this"),
      }),
    ),
  ).not.toThrow();

  expect(drops).toEqual([{ reason: "unmapped-slot", slot: 99 }]);
});

test("openTerminalSession: writeInput after disconnect is dropped-not-connected, never queued onto the wire", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const openPromise = client.openTerminalSession("term-t62d");
  const subscribeRequest = z
    .object({ type: z.literal("subscribe_terminal_request"), requestId: z.string() })
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  mock.triggerMessage(
  ...
  const outcome = session.writeInput(new Uint8Array([1]));
  expect(outcome).toBe("dropped-not-connected");
  expect(mock.sent).toHaveLength(0);
});

test("openTerminalSession: a daemon-reported terminal_stream_exit closes the session locally, with onClosed actually firing", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const openPromise = client.openTerminalSession("term-t62e");
  const subscribeRequest = z
    .object({ type: z.literal("subscribe_terminal_request"), requestId: z.string() })
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  mock.triggerMessage(
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      type: "terminal_stream_exit",
      payload: { terminalId: "term-t62e" },
    }),
  );

  expect(closedCount).toBe(1);
  expect(session.state).toBe("closed");
  // No redundant unsubscribe_terminal_request: the daemon already knows it exited.
  expect(mock.sent).toHaveLength(0);
});

// ============================================================================
// T110: queue modes (set_steering_mode_request / set_follow_up_mode_request /
// get_queue_modes_request). Fork/clone/rename have no such wire message to
// send — see the DISCLOSED GAP doc comment above `DaemonClient.buildAgentForkContext`
// in `daemon-client.ts` for why, and why this file adds no test claiming
// otherwise.
// ============================================================================

test("setSteeringMode sends set_steering_mode_request with the agent id and mode", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const modePromise = client.setSteeringMode("agt_1", "all");
  ...
  const request = z
    ...
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  expect(request.agentId).toBe("agt_1");
  ...
  await expect(modePromise).resolves.toBeNull();
});

test("setSteeringMode resolves the response's notice back to the caller", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const modePromise = client.setSteeringMode("agt_1", "one-at-a-time");
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        accepted: true,
        error: null,
        notice: { type: "warning", message: "Pi ignored the mode change" },
      },
    }),
  );

  await expect(modePromise).resolves.toEqual({
    type: "warning",
    message: "Pi ignored the mode change",
  });
});

test("setSteeringMode rejects when the daemon reports accepted: false", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  await expect(modePromise).rejects.toThrow("no such agent");
});

test("setFollowUpMode sends set_follow_up_mode_request with the agent id and mode", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const modePromise = client.setFollowUpMode("agt_2", "one-at-a-time");
  ...
  const request = z
    ...
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  expect(request.agentId).toBe("agt_2");
  expect(request.mode).toBe("one-at-a-time");

  mock.triggerMessage(
    wrapSessionMessage({
      type: "set_follow_up_mode_response",
      payload: { requestId: request.requestId, agentId: "agt_2", accepted: true, error: null },
    }),
  );
...
test("setFollowUpMode rejects when the daemon reports accepted: false", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const modePromise = client.setFollowUpMode("agt_2", "all");
  const request = parseSentFrame(mock.sent[mock.sent.length - 1]) as { requestId: string };
  ...
  await expect(modePromise).rejects.toThrow("provider not ready");
});

test("getQueueModes sends get_queue_modes_request and resolves both modes", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const modesPromise = client.getQueueModes("agt_3");
  ...
  const request = z
    ...
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  expect(request.agentId).toBe("agt_3");
...
test("getQueueModes rejects when the daemon reports an error", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  await expect(modesPromise).rejects.toThrow("no such agent");
});

// T131: same request/response contract as setSteeringMode/getQueueModes
// above, for auto-compaction.
test("setAutoCompaction sends set_auto_compaction_request with the agent id and enabled flag", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const compactionPromise = client.setAutoCompaction("agt_4", false);
  ...
  const request = z
    ...
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  expect(request.agentId).toBe("agt_4");
  expect(request.enabled).toBe(false);

  mock.triggerMessage(
    wrapSessionMessage({
      type: "set_auto_compaction_response",
      payload: { requestId: request.requestId, agentId: "agt_4", accepted: true, error: null },
    }),
  );

  await expect(compactionPromise).resolves.toBeNull();
});

test("setAutoCompaction resolves the response's notice back to the caller", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const compactionPromise = client.setAutoCompaction("agt_4", true);
  const request = parseSentFrame(mock.sent[mock.sent.length - 1]) as { requestId: string };
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        accepted: true,
        error: null,
        notice: { type: "warning", message: "Pi ignored the compaction change" },
      },
    }),
  );

  await expect(compactionPromise).resolves.toEqual({
    type: "warning",
    message: "Pi ignored the compaction change",
  });
});

test("setAutoCompaction rejects when the daemon reports accepted: false", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  await expect(compactionPromise).rejects.toThrow("no such agent");
});

test("getAutoCompaction sends get_auto_compaction_request and resolves the value", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
  ...
  const enabledPromise = client.getAutoCompaction("agt_5");
  ...
  const request = z
    ...
    .parse(parseSentFrame(mock.sent[mock.sent.length - 1]));
  expect(request.agentId).toBe("agt_5");
...
test("getAutoCompaction rejects when the daemon reports an error", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
...
test("getAutoCompaction rejects when the daemon reports no error but no known value", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
    clientId: "clsk_unit_test",
