...
import type { AgentAttentionNotificationPayload } from "@picompanion/protocol/agent-attention-notification";
import {
...
import type { MutableDaemonConfig, MutableDaemonConfigPatch } from "@picompanion/protocol/messages";
import { isRelayClientWebSocketUrl } from "@picompanion/protocol/daemon-endpoints";
import { terminalSubscriptionKey } from "@picompanion/protocol/terminal-subscription-key";
import {
  asUint8Array,
...
import {
  ...
  type TerminalStreamDropReason,
} from "./terminal-stream-router.js";
import { TerminalSessionRegistry, type TerminalSessionHandle } from "./terminal-session.js";
import type {
  BrowserAutomationExecuteRequest,
  BrowserAutomationExecuteResponse,
} from "@picompanion/protocol/browser-automation/rpc-schemas";
...
export interface SendMessageOptions {
  ...
  /**
   * T38B1b's per-message routing choice: `"steer"` interrupts the running
   * turn, `"followUp"` queues behind it. Added by the P6-W7 merge gate --
   * T38B1b built the whole web control against this option and
...
export class DaemonClient {
  ...
  /**
   * Session-shaped public terminal binary API (T62) — see
   * `terminal-session.ts`'s module doc for what this exposes instead of
   * the raw `sendBinaryFrame`/`tryHandleBinaryFrame` plumbing below, and
   * why. Built entirely on the same `terminalStreams`/`subscribeTerminal`/
   * `unsubscribeTerminal` primitives every other terminal caller here
   * already uses.
   */
  private readonly terminalSessions = new TerminalSessionRegistry({
    isConnected: () => this.isConnected,
    subscribe: async (terminalId) => {
      const payload = await this.subscribeTerminal(terminalId);
      return { error: payload.error ?? null };
    },
    unsubscribe: (terminalId) => this.unsubscribeTerminal(terminalId),
    sendInput: (terminalId, data) => this.sendTerminalSessionInput(terminalId, data),
    sendResize: (terminalId, rows, cols, intent) =>
      this.sendTerminalSessionResize(terminalId, rows, cols, intent),
    onStreamEvent: (handler) => this.terminalStreams.onEvent(handler),
    onRemoteExit: (handler) =>
      this.on("terminal_stream_exit", (message) => handler(message.payload.terminalId)),
  });
  ...
  /**
   * `TerminalSessionRegistry`'s only route onto the wire for input. Never
   * logs `data` or its length — see `terminal-session.ts`'s module doc.
   * `TerminalSessionImpl.checkWritable` already gates every call here on
   * `isConnected`, so by the time this runs `sendBinaryFrame`'s own
   * connected check is true in the same synchronous tick; the `if
   * (frame)` guard below only covers the (JS-single-threaded-impossible
   * in practice, but cheap to handle) case of `encodeInputFrame` finding
   * no slot despite that.
   */
  private sendTerminalSessionInput(terminalId: string, data: Uint8Array | string): void {
    const frame = this.terminalStreams.encodeInputFrame(terminalId, data);
    if (frame) {
      this.sendBinaryFrame(frame);
    }
  }

  /** Resize counterpart of `sendTerminalSessionInput` — see its doc. */
  private sendTerminalSessionResize(
    terminalId: string,
    rows: number,
    cols: number,
    intent?: "claim" | "update",
  ): void {
    const frame = this.terminalStreams.encodeInput(terminalId, {
      type: "resize",
      rows,
      cols,
      ...(intent ? { intent } : {}),
  ...
  /**
   * Sends `unregister_push_token` (T61, `packages/protocol/src/messages.ts`'s
   * `UnregisterPushTokenMessageSchema`) so a superseded or revoked token
   * stops receiving pushes. Fire-and-forget, exactly like
   * {@link registerPushToken} above — no response message, no request id
   * (`sendSessionMessage`, not `sendRequest`), so this never confirms the
   * daemon actually held `token`. The daemon deliberately makes
   * deregistering an unknown token a silent no-op (see the schema's own
   * doc comment and `session.unregister-push-token.test.ts` in
   * `packages/server`) precisely so a caller here can never distinguish
   * "that token was registered and I just removed it" from "that token
   * was never registered" — this method's own shape (`void`, no return
   * value to inspect) preserves that indistinguishability on the client
   * side too: nothing about calling this differs by what the daemon
   * actually held.
   */
  ...
  /**
   * T110: sends `set_steering_mode_request` (`packages/protocol/src/messages.ts`),
   * dispatched by `packages/server/src/server/session.ts`'s
   * `handleSetSteeringModeRequest` into `AgentManager.setSteeringMode`. Mirrors
   * `setAgentMode`'s request/response shape exactly — same
  ...
  async setSteeringMode(agentId: string, mode: QueueMode): Promise<AgentProviderNotice | null> {
    ...
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "set_steering_mode_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    if (!payload.accepted) {
      throw new Error(payload.error ?? "setSteeringMode rejected");
    }
    return payload.notice ?? null;
  }

  /** T110: same contract as `setSteeringMode`, for `set_follow_up_mode_request`. */
  async setFollowUpMode(agentId: string, mode: QueueMode): Promise<AgentProviderNotice | null> {
    ...
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "set_follow_up_mode_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    if (!payload.accepted) {
      throw new Error(payload.error ?? "setFollowUpMode rejected");
    }
    return payload.notice ?? null;
  }

  /**
   * T110: sends `get_queue_modes_request`, handled by
   * `handleGetQueueModesRequest` which reads both modes fresh off the live
   * provider session (see that handler's doc). Unlike the two setters above,
   * the response payload carries no `accepted` field — only `error` — so a
   * rejected request throws directly rather than checking `accepted`.
   */
  async getQueueModes(
    agentId: string,
    ...
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "get_queue_modes_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    return { steeringMode: payload.steeringMode, followUpMode: payload.followUpMode };
  }

  /**
   * T131: sends `set_auto_compaction_request` (`packages/protocol/src/messages.ts`),
   * dispatched by `packages/server/src/server/session.ts`'s
   * `handleSetAutoCompactionRequest` into `AgentManager.setAutoCompaction`.
   * Mirrors `setSteeringMode`'s request/response shape exactly — same
   * `AgentActionResponsePayloadSchema` envelope (`accepted`/`error`/`notice`).
   */
  async setAutoCompaction(agentId: string, enabled: boolean): Promise<AgentProviderNotice | null> {
    const requestId = this.createRequestId();
    const message = SessionInboundMessageSchema.parse({
      type: "set_auto_compaction_request",
      agentId,
      enabled,
      requestId,
    });
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "set_auto_compaction_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    if (!payload.accepted) {
      throw new Error(payload.error ?? "setAutoCompaction rejected");
    }
    return payload.notice ?? null;
  }

  /**
   * T131: sends `get_auto_compaction_request`, handled by
   * `handleGetAutoCompactionRequest` which reads the value fresh off the live
   * provider session (see that handler's doc). Unlike `setAutoCompaction`
   ...
   * mirroring `getQueueModes`. Throws (rather than resolving `null`) when the
   * daemon reports no error but also no known value, since this method's
   * return type must satisfy `SettingsClient.getAutoCompaction`'s
   * `Promise<boolean>` contract (`apps/web/src/features/settings/settings-client.ts`).
   */
  async getAutoCompaction(agentId: string): Promise<boolean> {
    const requestId = this.createRequestId();
    const message = SessionInboundMessageSchema.parse({
      type: "get_auto_compaction_request",
      agentId,
      requestId,
    });
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "get_auto_compaction_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    if (payload.enabled === null) {
      throw new Error("Daemon could not determine auto-compaction state");
    }
  ...
  async uploadFile(input: FileUploadInput): Promise<FileUploadResult> {
    ...
    if (bytes.byteLength > MAX_UPLOAD_BYTES) {
      throw new Error("File too large — max 100 MB over mobile");
    }
  ...
  /**
   * T163: cancels the in-flight `uploadFile()` call that was given
   * `uploadRequestId` as its `requestId`. Resolves only after the daemon
   * confirms the discard (`cancelled: true`) — `cancelled: false` means
   * nothing was pending for that id (it already completed, or the id is
   * unknown), and the caller must not treat that as a successful cancel.
   */
  async cancelUpload(uploadRequestId: string): Promise<FileUploadCancelResult> {
    return this.sendCorrelatedSessionRequest({
      message: { type: "file.upload.cancel.request", uploadRequestId },
      responseType: "file.upload.cancel.response",
    });
  }
  ...
  async listTrustedDevices(options?: { requestId?: string }): Promise<{
    requestId: string;
    devices: Array<{
      clientId: string;
    ...
    return this.sendNamespacedCorrelatedSessionRequest<"trusted_device.list.response">({
      requestId: options?.requestId,
      message: {
        type: "trusted_device.list.request",
      },
    });
  }
  ...
  async revokeTrustedDevice(
    ...
    return this.sendNamespacedCorrelatedSessionRequest<"trusted_device.revoke.response">({
      requestId: options?.requestId,
      message: {
        type: "trusted_device.revoke.request",
        clientId,
      },
    });
  }
  ...
  /**
   * Opens a session-scoped terminal binary channel for `terminalId`:
   * subscribes to its stream (reusing an already-open or in-flight
   * session for the same id rather than subscribing twice — see
   * `terminal-session.ts`'s module doc) and hands back a
   * `TerminalSessionHandle` for write/resize/close plus
   * output/snapshot/restore delivery. Rejects if the daemon's subscribe
   * RPC itself reports an error (e.g. an unknown terminal id).
   */
  openTerminalSession(terminalId: string): Promise<TerminalSessionHandle> {
    return this.terminalSessions.open(terminalId);
  }

  /**
   * Fires when an inbound terminal binary frame cannot be routed to any
   * terminal — an unmapped slot, which covers both a terminal never
   * opened on this connection and one already closed locally (see
   * `TerminalStreamRouter`'s `TerminalStreamDropReason` doc). Carries
   * only the drop reason and the raw multiplexing slot (0-255), never a
   * terminal id, a byte count, or any payload content.
   */
  onTerminalFrameDropped(
    handler: (reason: TerminalStreamDropReason, slot: number) => void,
  ): () => void {
    return this.terminalStreams.onDrop(handler);
  }
