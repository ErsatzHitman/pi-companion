...
describe("agent deep links", () => {
  it("round-trips an existing agent target", () => {
    ...
    expect(link).toBe("picompanion://h/server%2Fmain/agent/agent%20123");
  ...
  it("rejects links outside the exact agent route", () => {
    ...
    expect(parseAgentDeepLink("picompanion://app/h/server/agent/agent-1")).toBeNull();
    expect(parseAgentDeepLink("picompanion://h/server/agent/agent-1?message=hello")).toBeNull();
    expect(parseAgentDeepLink("picompanion://h/server/agent/agent-1/extra")).toBeNull();
  });
});
