...
// Referenced to satisfy noUnusedLocals (PiUiElementSchema imported per spec)
void PiUiElementSchema;
// ---------------------------------------------------------------------------
...
const ToolCallDetailPayloadSchema: z.ZodType<ToolCallDetail, unknown> = z.discriminatedUnion(
  ...
  [
    ...
    z.object({
      ...
      edits: z.array(z.object({ oldString: z.string(), newString: z.string() })).optional(),
    }),
    z.object({
...
// T52A1: message-level images are referenced (a materialized local file
// path + mimeType), never the raw base64 bytes — keep this schema literal
// (not `.passthrough()`) so an accidentally-inlined data blob is rejected
// by validation rather than silently forwarded to every timeline fetch.
const AgentTimelineImageRefPayloadSchema: z.ZodType<AgentTimelineImageRef, unknown> = z.object({
  mimeType: z.string(),
  path: z.string(),
  bytes: z.number().optional(),
});
...
export const AgentTimelineItemPayloadSchema: z.ZodType<AgentTimelineItem, unknown> = z.union([
  z.object({
    ...
    images: z.array(AgentTimelineImageRefPayloadSchema).optional(),
  }),
  z.object({
    type: z.literal("assistant_message"),
    text: z.string(),
    messageId: z.string().optional(),
    replaceMessageId: z.string().optional(),
  ...
  z.object({
    ...
    // T143: Pi's own compaction result fields, carried once the daemon has
    // structured data (`CompactionTimelineItem` in `./agent-types.ts`).
    summary: z.string().optional(),
  ...
  z.object({
    type: z.literal("pi_ui_snapshot"),
    state: PiUiStateSchema,
  }),
]);

// The connected client whose answer a daemon resolution reflects (T111,
// plan.md §12.3). Optional and additive on purpose: an older daemon or an
// auto-resolution with no specific answering client (e.g. an auto-deny when
// an agent's session ends while a request is still pending) omits it
// entirely, and every consumer must treat that exactly like the field never
// having existed — see `AgentPermissionResolvedMessageSchema` further below,
// this schema's own two `pi.ui.action.response`/`pi_ui_action_result` uses
// (T128), and `packages/frontend-core/src/actions/arbitration.ts`'s
// `AnsweredBy`, which this mirrors field-for-field so any of these wire
// payloads can be handed to it as-is. Declared here (ahead of
// `AgentStreamEventPayloadSchema` below, which references it) rather than
// alongside `AgentPermissionResolvedMessageSchema` further down the file,
// because a `const` referenced before its module-scope initializer throws at
// import time — this schema now has two use sites on opposite sides of that
// one.
export const PermissionAnsweredBySchema = z.object({
  clientId: z.string().optional(),
  label: z.string().optional(),
});
export type PermissionAnsweredBy = z.infer<typeof PermissionAnsweredBySchema>;
...
export const AgentStreamEventPayloadSchema = z.discriminatedUnion("type", [
  ...
  // Hand-synced with AgentStreamEvent in packages/server/src/server/agent/agent-sdk-types.ts and packages/protocol/src/agent-types.ts — keep identical.
  z.object({
    type: z.literal("pi_queue_update"),
    provider: AgentProviderSchema,
  ...
  z.object({
    type: z.literal("pi_retry"),
    provider: AgentProviderSchema,
    phase: z.enum(["assistant", "compaction", "branchSummary"]),
  ...
  z.object({
    type: z.literal("pi_composer"),
    provider: AgentProviderSchema,
    text: z.string(),
    turnId: z.string().optional(),
  }),
  z.object({
    type: z.literal("pi_ui_state"),
    provider: AgentProviderSchema,
    state: PiUiStateSchema,
  }),
  z.object({
    type: z.literal("pi_ui_delta"),
    provider: AgentProviderSchema,
    agentId: z.string().min(1),
    revision: z.number().int().nonnegative(),
    delta: PiUiDeltaSchema,
  }),
  z.object({
    type: z.literal("pi_ui_action_result"),
    provider: AgentProviderSchema,
    result: PiUiActionResultSchema,
    // The connected client whose dispatched `pi.ui.action.request` this
    // result reflects (T128, mirroring T111's `agent_permission_resolved`
    // `answeredBy`). Optional and additive: absent entirely whenever the
    // daemon does not know who dispatched the action that produced this
    // result. See `PermissionAnsweredBySchema`'s doc comment above.
    answeredBy: PermissionAnsweredBySchema.optional(),
  }),
]);
...
// Per-message queue routing, mirrored from Pi's `prompt` RPC command
// (`streamingBehavior?: "steer" | "followUp"` in the installed Pi's
// dist/modes/rpc/rpc-types.d.ts). Omitting this field on a prompt message
// preserves today's behaviour exactly: Pi enqueues the message using the
// session's current steering/follow-up mode, same as before this field
// existed. See QueueModeSchema below for the separate all/one-at-a-time
// mode setting this field routes against.
export const PromptStreamingBehaviorSchema = z.enum(["steer", "followUp"]);
export type PromptStreamingBehavior = z.infer<typeof PromptStreamingBehaviorSchema>;
...
export const SendAgentMessageSchema = z.object({
  ...
  // OPTIONAL: routes this individual message as a steer or a follow-up,
  // independent of the session's steering/follow-up mode. Absent by default;
  // omitting it changes nothing about how the message is queued.
...
export const SendAgentMessageRequestSchema = z.object({
  ...
  images: z.array(ImageAttachmentSchema).optional(),
  attachments: AgentAttachmentsSchema,
  // OPTIONAL: routes this individual message as a steer or a follow-up. See
  // PromptStreamingBehaviorSchema above — omitting it preserves today's
  // behaviour exactly.
  streamingBehavior: PromptStreamingBehaviorSchema.optional(),
});
...
// ============================================================================
//
// Mirrors Pi's `set_steering_mode` / `set_follow_up_mode` RPC commands and the
// `steeringMode` / `followUpMode` fields of `get_state`'s response (verified
// in the installed Pi: dist/modes/rpc/rpc-mode.js:405-412 handles both set
// commands; RpcSessionState in dist/modes/rpc/rpc-types.d.ts carries both
// fields). "all" delivers every queued message together; "one-at-a-time" (the
// default for both modes) delivers one per cycle. This is the session-wide
// mode setting — distinct from the per-message PromptStreamingBehaviorSchema
// above, which routes a single message regardless of the current mode.
export const QueueModeSchema = z.enum(["all", "one-at-a-time"]);
export type QueueMode = z.infer<typeof QueueModeSchema>;

export const SetSteeringModeRequestMessageSchema = z.object({
  type: z.literal("set_steering_mode_request"),
  agentId: z.string(),
  mode: QueueModeSchema,
  requestId: z.string(),
});

export const SetSteeringModeResponseMessageSchema = z.object({
  type: z.literal("set_steering_mode_response"),
  payload: AgentActionResponsePayloadSchema,
});

export const SetFollowUpModeRequestMessageSchema = z.object({
  type: z.literal("set_follow_up_mode_request"),
  agentId: z.string(),
  mode: QueueModeSchema,
  requestId: z.string(),
});

export const SetFollowUpModeResponseMessageSchema = z.object({
  type: z.literal("set_follow_up_mode_response"),
  payload: AgentActionResponsePayloadSchema,
});

// Pi has no dedicated "get" RPC for these; it reads both modes off the single
// get_state response (RpcSessionState.steeringMode / .followUpMode). We mirror
// that shape as one request/response pair covering both modes, rather than
// two separate round trips.
export const GetQueueModesRequestMessageSchema = z.object({
  type: z.literal("get_queue_modes_request"),
  agentId: z.string(),
  requestId: z.string(),
});

export const GetQueueModesResponseMessageSchema = z.object({
  type: z.literal("get_queue_modes_response"),
  payload: z.object({
    requestId: z.string(),
    agentId: z.string(),
    steeringMode: QueueModeSchema.nullable(),
    followUpMode: QueueModeSchema.nullable(),
    error: z.string().nullable(),
  }),
});

// ============================================================================
// Auto-compaction (T131)
// ============================================================================
//
...
export const SetAutoCompactionResponseMessageSchema = z.object({
  type: z.literal("set_auto_compaction_response"),
  payload: AgentActionResponsePayloadSchema,
});

export const GetAutoCompactionRequestMessageSchema = z.object({
  type: z.literal("get_auto_compaction_request"),
  agentId: z.string(),
  requestId: z.string(),
});

export const GetAutoCompactionResponseMessageSchema = z.object({
  type: z.literal("get_auto_compaction_response"),
  payload: z.object({
    requestId: z.string(),
    agentId: z.string(),
...
// T163: `uploadRequestId` names the in-flight upload to discard — the
// `requestId` this call was originally sent with (the `FileUploadRequest`
// and the `FileBegin`/`FileChunk`/`FileEnd` frames all share it). That is
// deliberately a separate field from this message's own `requestId`,
// which correlates this cancel call's request/response pair the same way
// every other correlated RPC in this file does; reusing the upload's id
// for both would collide with the still-pending `file.upload.response`.
export const FileUploadCancelRequestSchema = z.object({
  type: z.literal("file.upload.cancel.request"),
 ...
 * reinstall) and wants the old one to stop must send
...
/**
 * Explicitly removes one push token from the daemon's `PushTokenStore`
 * so it stops receiving notifications. See the contract note above
 * `RegisterPushTokenMessageSchema` — this is the only way to make a
 * superseded or revoked token stop receiving before Expo itself reports
 * it dead.
 *
 * Deregistering a token the store never held (already removed, expired,
 * or simply unknown) is a silent no-op, not an error: the daemon must
 * never let a client distinguish "that token existed and I just removed
 * it" from "that token was never registered" — either would leak
 * whether some other client/device had registered it. See
 * `PushTokenStore.removeToken` (only acts, and only logs, when the
 * token was actually present) and
 * `session.unregister-push-token.test.ts` for both directions asserted.
 */
export const UnregisterPushTokenMessageSchema = z.object({
  type: z.literal("unregister_push_token"),
  token: z.string(),
});
...
export const TrustedDeviceListRequestSchema = z.object({
  type: z.literal("trusted_device.list.request"),
  requestId: z.string().min(1),
});

export const TrustedDeviceRevokeRequestSchema = z.object({
  type: z.literal("trusted_device.revoke.request"),
  requestId: z.string().min(1),
...
export const SessionInboundMessageSchema = z.discriminatedUnion("type", [
  ...
  LoopStopRequestSchema,
  z.object({
    type: z.literal("pi.ui.action.request"),
    agentId: z.string().min(1),
  ...
  z.object({
    type: z.literal("pi.ui.subscribe.request"),
  ...
  .object({
    ...
    features: z
      .object({
        ...
        piUiBridge: z.boolean().optional(),
        // COMPAT(piUiPayloadV2): added 2026-08-31, remove after 2027-02-28.
        // Advertises that the daemon emits the canonical typed `payload` on Pi
        // UI elements. Separate from piUiBridge, which only says the bridge
        // exists; clients must not infer payload support from it.
        piUiPayloadV2: z.boolean().optional(),
        // COMPAT(trustedDevices): Phase 7 trusted-device list
        trustedDevices: z.boolean().optional(),
      })
      .optional(),
  })
  .passthrough()
...
// `PermissionAnsweredBySchema`/`PermissionAnsweredBy` are declared near the
// top of this file, ahead of `AgentStreamEventPayloadSchema` — see that
...
export const AgentPermissionResolvedMessageSchema = z.object({
  ...
  payload: z.object({
    ...
    requestId: z.string(),
    resolution: AgentPermissionResponseSchema,
    // Absent entirely (not `undefined`-with-a-key) on the wire whenever the
    // daemon does not know who answered — see the schema's own doc comment.
    answeredBy: PermissionAnsweredBySchema.optional(),
  }),
});
...
// T163: `cancelled: true` is only sent once the daemon has actually
// discarded every staged byte for `uploadRequestId` (see
// `FileUploadStore.cancelUpload`) — a client must not report "cancelled"
// on the strength of having sent this request alone. `cancelled: false`
// covers both "nothing was pending" (the upload already completed, or the
// id was never known) and any discard failure; `error` explains which.
export const FileUploadCancelResponseSchema = z.object({
  type: z.literal("file.upload.cancel.response"),
  payload: z.object({
    requestId: z.string(),
...
export const TrustedDeviceListResponseSchema = z.object({
  type: z.literal("trusted_device.list.response"),
  payload: z.object({
    requestId: z.string(),
    devices: z.array(
      z.object({
...
export const TrustedDeviceRevokeResponseSchema = z.object({
  type: z.literal("trusted_device.revoke.response"),
  payload: z.object({
    requestId: z.string(),
    clientId: z.string(),
    success: z.boolean(),
...
export const SessionOutboundMessageSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("pi.ui.action.response"),
    payload: z.object({
      requestId: z.string().min(1),
      ok: z.boolean(),
      error: z.string().nullable(),
      // The connected client this synchronous RPC ack is being sent to —
      // i.e. the one whose `pi.ui.action.request` produced it (T128, same
      // shape as `pi_ui_action_result`'s `answeredBy` above and
      // `agent_permission_resolved`'s since T111). Optional and additive:
      // absent entirely for an older daemon.
      answeredBy: PermissionAnsweredBySchema.optional(),
    }),
  }),
]);
...
export type SetSteeringModeResponseMessage = z.infer<typeof SetSteeringModeResponseMessageSchema>;
export type SetFollowUpModeResponseMessage = z.infer<typeof SetFollowUpModeResponseMessageSchema>;
export type GetQueueModesResponseMessage = z.infer<typeof GetQueueModesResponseMessageSchema>;
export type SetAutoCompactionResponseMessage = z.infer<
  typeof SetAutoCompactionResponseMessageSchema
>;
...
export type SetSteeringModeRequestMessage = z.infer<typeof SetSteeringModeRequestMessageSchema>;
export type SetFollowUpModeRequestMessage = z.infer<typeof SetFollowUpModeRequestMessageSchema>;
...
export type UnregisterPushTokenMessage = z.infer<typeof UnregisterPushTokenMessageSchema>;
...
export const WSHelloMessageSchema = z.object({
  ...
  capabilities: z
    .object({
      ...
      // COMPAT(piUiPayloadV2): added 2026-08-31, remove after 2027-02-28.
      [CLIENT_CAPS.piUiPayloadV2]: z.boolean().optional(),
