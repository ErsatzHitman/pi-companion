...
describe("paseo worktree manager", () => {
  ...
  it("treats a worktree as paseo-owned even when its .git admin is missing", async () => {
    ...
    await expect(
      isPaseoOwnedWorktreeCwd(join(created.worktreePath, "packages", "core"), { paseoHome }),
  ...
  it("maps only root-contained workspace paths into a replacement worktree", () => {
    ...
    const nestedWorkspaceCwd = join(sourceWorktreePath, "packages", "core");
    ...
    ).toBe(join(targetWorktreePath, "packages", "core"));
    expect(() =>
      mapWorkspaceCwdToWorktree({
        sourceWorktreePath,
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const sourceWorktreePath = join(tempDir, "source-worktree");
      const workspaceCwd = join(sourceWorktreePath, "packages", "core");
