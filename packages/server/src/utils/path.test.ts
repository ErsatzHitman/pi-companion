...
describe("path equivalence", () => {
  ...
  test("checks POSIX root containment without prefix false positives", () => {
    expect(isPathInsideRoot("/opt/paseo", "/opt/paseo/node_modules/@picompanion/server")).toBe(
      true,
    );
  ...
  test("checks Windows root containment case-insensitively", () => {
    expect(
      isPathInsideRoot("C:\\Paseo\\node_modules", "c:/paseo/node_modules/@picompanion/server"),
    ).toBe(true);
  ...
  test.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      try {
        ...
        const nestedPath = join(realRoot, "packages", "core");
        const aliasRoot = join(tempDir, "root-alias");
        mkdirSync(nestedPath, { recursive: true });
        symlinkSync(realRoot, aliasRoot, "dir");

        expect(getRealpathAwareRelativePath(aliasRoot, nestedPath)).toBe(join("packages", "core"));
