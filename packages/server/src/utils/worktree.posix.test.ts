...
describe.skipIf(isPlatform("win32"))("worktree POSIX-only", () => {
  describe("createWorktree", () => {
    ...
    it("creates and owns worktrees under a configured root", async () => {
      ...
      expect(worktrees.map((entry: PaseoWorktreeInfo) => entry.path)).toContain(
        result.worktreePath,
      );
    ...
    it("streams setup command progress events while commands are executing", async () => {
      ...
      const results = await runWorktreeSetupCommands({
        ...
        branchName: "main",
        cleanupOnFailure: false,
        onEvent: (event: WorktreeSetupCommandProgressEvent) => {
      ...
      const paseoConfig = {
        worktree: {
          terminals: [
            ...
            { command: "cd packages/core && npm run dev" },
          ],
        },
      };
      writeFileSync(join(repoDir, "paseo.json"), JSON.stringify(paseoConfig));
      ...
      const paseoConfig = {
        worktree: {
          terminals: [
            ...
            {},
            { name: "   ", command: "   " },
            { name: " Watch ", command: "npm run watch", cwd: "packages/core" },
  ...
  describe("paseo worktree manager", () => {
    ...
    it("isolates worktree roots for repositories that share the same directory name", async () => {
      ...
      expect(repoAWorktrees.map((entry: PaseoWorktreeInfo) => entry.path)).toEqual([
        fromRepoA.worktreePath,
      ]);
      expect(repoBWorktrees.map((entry: PaseoWorktreeInfo) => entry.path)).toEqual([
    ...
    it("lists and deletes paseo worktrees under ~/.paseo/worktrees/{hash}", async () => {
      ...
      const paths = worktrees.map((worktree: PaseoWorktreeInfo) => worktree.path).sort();
      ...
      expect(remaining.map((worktree: PaseoWorktreeInfo) => worktree.path)).toEqual([
        second.worktreePath,
      ]);
    });
    ...
    it("deletes a paseo worktree even when given a subdirectory path", async () => {
      ...
      const remaining = await listPaseoWorktrees({ cwd: repoDir, paseoHome });
      expect(
        remaining.some((worktree: PaseoWorktreeInfo) => worktree.path === created.worktreePath),
      ).toBe(false);
    });
