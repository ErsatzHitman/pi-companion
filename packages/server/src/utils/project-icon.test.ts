...
describe("findProjectIcon", () => {
  ...
  describe("monorepo package directories", () => {
    it("finds icon in packages/*/public directory", async () => {
      mkdirSync(join(tempDir, "packages", "core", "public"), { recursive: true });
      writeFileSync(join(tempDir, "packages", "core", "public", "favicon.ico"), "icon");

      const result = await findProjectIcon(tempDir);
      expect(result).toBe(join(tempDir, "packages", "core", "public", "favicon.ico"));
    });
    ...
    it("prioritizes root priority dirs over monorepo dirs", async () => {
      ...
      writeFileSync(join(tempDir, "packages", "core", "public", "favicon.ico"), "package icon");
