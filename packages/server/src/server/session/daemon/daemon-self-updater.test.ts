...
const cliPackagePath = `${globalNodeModules}/@picompanion/cli`;
const npmServerPackageRoot = `${cliPackagePath}/node_modules/@picompanion/server`;
...
describe("DaemonSelfUpdater", () => {
  ...
  test("does not run install when npm global cli is missing", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [new Error("@picompanion/cli is not installed with npm -g on this host")],
    });

    const { result, phases } = await runUpdate({ runtime });

    expect(result.success).toBe(false);
    expect(result.error).toBe("@picompanion/cli is not installed with npm -g on this host");
  ...
  test("does not update a daemon whose version does not match the npm global cli", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        "This daemon is not running from the npm global @picompanion/cli install (global npm has 0.1.15, daemon is 0.1.96).",
      newVersion: null,
    });
    expect(calls).toEqual(["inspect"]);
  });
  ...
  test("does not update a daemon running outside the npm global package tree", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error: "This daemon is not running from the npm global @picompanion/cli install.",
      newVersion: null,
    });
    expect(calls).toEqual(["inspect"]);
  });
  ...
  test("does not update linked global installs", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        "The global @picompanion/cli install is linked; self-update only supports normal npm global installs.",
      newVersion: null,
    });
  });
