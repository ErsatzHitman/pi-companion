...
export class WorkspaceFilesSession {
  ...
  async handleFileWriteRequest(request: FileWriteRequest): Promise<void> {
    ...
    if (!cwd) {
      this.host.emit({
        ...
        payload: {
          result: { status: "error", error: "cwd is required" },
          requestId: request.requestId,
        },
      });
      return;
  ...
  handleFileUploadRequest(request: FileUploadRequest): void {
    const rejected = this.fileUploads.beginUpload(request);
    if (rejected) {
      this.host.emit(rejected);
    }
  }

  /** T163: closes the upload-cancel protocol gap `use-file-upload.ts` disclosed. */
  async handleFileUploadCancelRequest(request: FileUploadCancelRequest): Promise<void> {
    const { requestId, uploadRequestId } = request;
    const result = await this.fileUploads.cancelUpload(uploadRequestId);
    this.host.emit({
