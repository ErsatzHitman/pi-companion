...
describe("file uploads", () => {
  ...
  it("cancelUpload discards staged chunks from disk and reports cancelled: true (T163)", async () => {
    const paseoHome = makePaseoHome();
    const uploads = new FileUploadStore({ paseoHome });
    ...
    await expect(uploads.receiveFrame(uploadBegins("req-cancel"))).resolves.toBeNull();
    await expect(uploads.receiveFrame(uploadChunk("req-cancel", "hello"))).resolves.toBeNull();

    const uploadDir = join(paseoHome, "uploads", "upload_req-cancel");
    const path = join(uploadDir, "notes.txt");
    expect(existsSync(path)).toBe(true);

    await expect(uploads.cancelUpload("req-cancel")).resolves.toEqual({
      cancelled: true,
      error: null,
    });

    // The receipt proven here is the directory actually being gone from
    // disk by the time cancelUpload resolves — not merely a truthy result.
    expect(existsSync(uploadDir)).toBe(false);
    expect(existsSync(path)).toBe(false);

    // A FileEnd that arrives after the cancel must not resurrect the
    // upload: it is no longer pending, so this is a silent no-op.
    await expect(uploads.receiveFrame(uploadEnds("req-cancel"))).resolves.toBeNull();
    expect(existsSync(uploadDir)).toBe(false);
  });

  it("cancelUpload reports cancelled: false and leaves the file alone once the upload already completed (T163)", async () => {
    const paseoHome = makePaseoHome();
    const uploads = new FileUploadStore({ paseoHome });
    ...
    uploads.beginUpload({
      ...
      size: 11,
      modifiedAt: "2026-05-02T00:00:00.000Z",
      requestId: "req-too-late",
    });
    await expect(uploads.receiveFrame(uploadBegins("req-too-late"))).resolves.toBeNull();
    await expect(
      uploads.receiveFrame(uploadChunk("req-too-late", "hello world")),
    ).resolves.toBeNull();
    await expect(uploads.receiveFrame(uploadEnds("req-too-late"))).resolves.toMatchObject({
      payload: { error: null },
    });

    const path = join(paseoHome, "uploads", "upload_req-too-late", "notes.txt");
    expect(existsSync(path)).toBe(true);

    await expect(uploads.cancelUpload("req-too-late")).resolves.toEqual({
      cancelled: false,
      error: "No upload in progress for this id.",
    });
    // A completed upload's file must survive an after-the-fact cancel.
    expect(existsSync(path)).toBe(true);
  });

  it("cancelUpload reports cancelled: false for an id nothing was ever pending under (T163)", async () => {
    const paseoHome = makePaseoHome();
    const uploads = new FileUploadStore({ paseoHome });

    await expect(uploads.cancelUpload("req-never-existed")).resolves.toEqual({
      cancelled: false,
      error: "No upload in progress for this id.",
    });
  });

  it("cancelUpload waits for an in-flight chunk write before discarding it (T163)", async () => {
    const paseoHome = makePaseoHome();
    const uploads = new FileUploadStore({ paseoHome });
    ...
    await expect(uploads.receiveFrame(uploadBegins("req-race"))).resolves.toBeNull();

    // Queue a chunk write and cancel before it settles — cancelUpload must
    // not remove the directory out from under a write still in flight.
    const chunkPromise = uploads.receiveFrame(uploadChunk("req-race", "hello"));
    const cancelPromise = uploads.cancelUpload("req-race");

    await expect(chunkPromise).resolves.toBeNull();
    await expect(cancelPromise).resolves.toEqual({ cancelled: true, error: null });
    expect(existsSync(join(paseoHome, "uploads", "upload_req-race"))).toBe(false);
  });
