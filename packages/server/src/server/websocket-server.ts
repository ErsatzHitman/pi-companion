...
import { asUint8Array, decodeBinaryFrame } from "@picompanion/protocol/binary-frames/index";
...
} from "@picompanion/protocol/agent-attention-notification";
...
import type { BrowserAutomationExecuteResponse } from "@picompanion/protocol/browser-automation/rpc-schemas";
import {
  BrowserAutomationHostCapabilitySchema,
  type BrowserAutomationHostCapability,
} from "@picompanion/protocol/browser-automation/capabilities";
...
export class VoiceAssistantWebSocketServer {
  ...
  private buildServerInfoStatusPayload(): ServerInfoStatusPayload {
    return {
      ...
      features: {
        ...
        // The daemon normalizes v1 helper elements into a canonical typed
        // `payload` and emits it to clients that advertise the matching
        // client capability (plan.md §4.2).
        piUiPayloadV2: true,
  ...
  private touchTrustedConnection(activeConnection: SessionConnection): void {
    if (activeConnection.kind !== "trusted") return;
    activeConnection.lastSeenAt = new Date().toISOString();
  }

  private handleTrustedDeviceListRequest(
    ws: WebSocketLike,
    _activeConnection: TrustedSessionConnection,
    requestId: string,
  ): void {
    const devices = [...this.externalSessionsByKey.values()].map((conn) => ({
      clientId: conn.clientId,
      appVersion: conn.appVersion,
      lastSeenAt: conn.lastSeenAt,
      connected: conn.sockets.size > 0,
    }));
    this.sendToClient(
      ws,
      wrapSessionMessage({
        type: "trusted_device.list.response",
        payload: { requestId, devices },
      }),
    );
  }

  private async handleTrustedDeviceRevokeRequest(
    ws: WebSocketLike,
    ...
    if (!trimmed) {
      this.sendToClient(
        ws,
        wrapSessionMessage({
          type: "trusted_device.revoke.response",
          payload: {
            requestId,
    ...
    if (trimmed === activeConnection.clientId) {
      this.sendToClient(
        ws,
        wrapSessionMessage({
          type: "trusted_device.revoke.response",
    ...
    const target = this.externalSessionsByKey.get(trimmed);
    if (!target) {
      this.sendToClient(
        ws,
        wrapSessionMessage({
          type: "trusted_device.revoke.response",
          payload: { requestId, clientId: trimmed, success: false, error: "Device not found" },
        }),
      );
      return;
    }
    // Close sockets and cleanup session for revoked device
    for (const sock of [...target.sockets]) {
      try {
        sock.close(1000, "Revoked by trusted device list");
      } catch {}
    }
  ...
  private async dispatchSessionMessage(
    ...
    // Track last-seen for trusted devices and handle trusted-device RPCs in-place (not via Session)
    if (activeConnection.kind === "trusted") {
      this.touchTrustedConnection(activeConnection);
      if (message.message.type === "trusted_device.list.request") {
        this.handleTrustedDeviceListRequest(ws, activeConnection, message.message.requestId);
        return;
      }
  ...
  private async broadcastAgentAttention(params: {
    ...
    if (plan.shouldPush) {
      const isPermissionPush = params.reason === "permission" && Boolean(permissionRequest);
      const pushPayload = isPermissionPush
        ? {
            ...notification,
            categoryId: "permission-request",
            channelId: "permission-request",
