  ...
  private handleTickets(messages: ExpoPushMessage[], tickets: ExpoPushTicket[]): void {
    for (let i = 0; i < tickets.length; i++) {
      ...
      if (ticket.status === "error") {
        // Never log the raw token (a credential-shaped value): only its
        // length, which is enough to spot a malformed/truncated token
        // without letting the daemon's own logs become a second place a
        // push token can leak.
        this.logger.error(
          { tokenLength: message.to.length, message: ticket.message, details: ticket.details },
