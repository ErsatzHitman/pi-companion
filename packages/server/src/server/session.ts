...
import type { TerminalActivity } from "@picompanion/protocol/terminal-activity";
import type { BinaryFrame } from "@picompanion/protocol/binary-frames/index";
...
import { getErrorMessage, getErrorMessageOr } from "@picompanion/protocol/error-utils";
import { getAgentStatusPriority } from "@picompanion/protocol/agent-state-bucket";
...
} from "./agent/providers/pi/ui-bridge/payload-compat.js";
...
import { parseGitRemoteLocation } from "@picompanion/protocol/git-remote";
import {
...
import { buildPiUiActionEnvelope } from "./agent/providers/pi/ui-bridge/identity.js";
import type { PiUiStateStore } from "./agent/providers/pi/ui-bridge/state.js";

/**
 * Pi UI action routing needs the daemon's own element state to resolve a
 * composite `ns:id[#rowId]` identity, so the Pi session exposes its store.
 * Non-Pi sessions simply do not have one.
 */
function getPiUiStateStore(agent: unknown): PiUiStateStore | null {
  const session = (agent as { session?: unknown } | null)?.session;
  const accessor = (session as { getUiBridgeStateStore?: () => PiUiStateStore } | undefined)
...
export class Session {
   ...
   * COMPAT(piUiPayloadV2): added 2026-08-31, remove after 2027-02-28.
   *
   * Dual emission for Pi UI elements (plan.md §4.2, step 6). Clients that
   * negotiated `piUiPayloadV2` receive the canonical typed `payload`; every
   * other client keeps receiving the legacy v1 top-level projection for the
   * duration of the compatibility window. Non-Pi-UI events pass straight
   * through by reference.
   */
  private projectAgentStreamEventForPiUiPayload(
    serializedEvent: Extract<SessionOutboundMessage, { type: "agent_stream" }>["payload"]["event"],
    supportsPayloadV2: boolean,
  ): Extract<SessionOutboundMessage, { type: "agent_stream" }>["payload"]["event"] {
    if (serializedEvent.type === "pi_ui_state") {
      return {
        ...serializedEvent,
        state: projectPiUiStateForClient(serializedEvent.state, supportsPayloadV2),
      };
    }
    if (serializedEvent.type === "pi_ui_delta") {
      return {
        ...serializedEvent,
        delta: projectPiUiDeltaForClient(serializedEvent.delta, supportsPayloadV2),
      };
    }
    if (serializedEvent.type === "timeline" && serializedEvent.item.type === "pi_ui_snapshot") {
      return {
        ...serializedEvent,
        item: {
          ...serializedEvent.item,
          state: projectPiUiStateForClient(serializedEvent.item.state, supportsPayloadV2),
        },
      };
    }
  ...
  private forwardAgentStream(
    ...
    if (this.clientCapabilitiesBySource.size === 0 || !this.onMessageToSource) {
      // COMPAT(piUiPayloadV2): added 2026-08-31, remove after 2027-02-28.
      const serializedEvent = this.projectAgentStreamEventForPiUiPayload(
        rawSerializedEvent,
        this.supports(CLIENT_CAPS.piUiPayloadV2),
      );
    ...
    for (const [source, capabilities] of this.clientCapabilitiesBySource) {
      ...
      const serializedEvent = this.projectAgentStreamEventForPiUiPayload(
        rawSerializedEvent,
        capabilities.has(CLIENT_CAPS.piUiPayloadV2),
      );
        ...
        if (event.event.type === "permission_requested") {
          ...
          this.emit({
            ...
            payload: {
              ...
              // T111: forward whichever client's answer this resolution
              // reflects, when the arbitrator upstream knew it. Every
              // currently-connected `Session` (one per browser/mobile/CLI
              // client) receives this same `permission_resolved` stream
              // event and re-broadcasts it here, so a second client's
              // `agent_permission_resolved` push carries the same
              // `answeredBy` the first (answering) client's did.
  ...
  private dispatchPiUiMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    if (msg.type === "pi.ui.action.request") {
      const { agentId, elementId, actionId, payload, requestId } = msg;
      void (async () => {
        try {
          const agent = this.agentManager.getAgent(agentId);
          if (!agent) throw new Error(`Agent ${agentId} not found`);
          // Pi UI action routing resolves composite `ns:id[#rowId]` identity
          // against daemon state before dispatch, never a bare element id
          // (plan.md §4.2). A bare id is only honoured when exactly one
          // namespace owns it.
          const stateStore = getPiUiStateStore(agent);
          if (!stateStore) throw new Error(`Pi UI bridge is not active for agent ${agentId}`);
          const resolution = stateStore.resolveActionTarget(agentId, { elementId, actionId });
          if (!resolution.ok) throw new Error(resolution.error);
          const data = buildPiUiActionEnvelope({
          ...
          await sendPromptToAgent({
            ...
            agentStorage: this.agentStorage,
            agentId,
            prompt: `/pi_ui_event ${b64}`,
            logger: this.sessionLogger,
          });
          this.emit({
            type: "pi.ui.action.response",
            // T128: attribute this response to the connected client whose
            // pi.ui.action.request produced it, mirroring T111's
            // handleAgentPermissionResponse capturing this.clientId at the
            // real resolution site.
            payload: { requestId, ok: true, error: null, answeredBy: { clientId: this.clientId } },
          });
        } catch (err) {
          ...
          this.emit({
            ...
            payload: {
              ...
              ok: false,
              error: message,
              answeredBy: { clientId: this.clientId },
            },
          });
        }
      })();
    ...
    if (msg.type === "pi.ui.subscribe.request") {
      const { agentIds } = msg;
      for (const agentId of agentIds) {
        // Emit full state (empty if no Pi store yet) via agent_stream pi_ui_state
        this.emit({
          type: "agent_stream",
          payload: {
            agentId,
            event: {
              type: "pi_ui_state",
              provider: "pi",
              state: { agentId, revision: 0, elements: [], updatedAt: new Date().toISOString() },
            },
  ...
  private dispatchAgentConfigMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      ...
      case "set_steering_mode_request":
        return this.handleSetSteeringModeRequest(msg);
      case "set_follow_up_mode_request":
        return this.handleSetFollowUpModeRequest(msg);
      case "get_queue_modes_request":
        return this.handleGetQueueModesRequest(msg);
      case "set_auto_compaction_request":
  ...
  private dispatchWorkspaceFileMessage(
    ...
    switch (msg.type) {
      ...
      case "file_download_bytes_request":
        return this.workspaceFilesSession.handleFileDownloadBytesRequest(msg);
      ...
      case "file.upload.request":
        ...
        return undefined;
      case "file.upload.cancel.request":
        return this.workspaceFilesSession.handleFileUploadCancelRequest(msg);
      default:
        return undefined;
    }
  }
  ...
  private async dispatchMiscMessage(msg: SessionInboundMessage): Promise<void> {
    switch (msg.type) {
      ...
      case "unregister_push_token":
        this.handleUnregisterPushToken(msg.token);
        return;
    }
  }
  ...
  /**
   * T38B0c: set the session-wide steering mode ("all" delivers every queued
   * message together while a turn runs; "one-at-a-time" delivers one per
   * cycle). AgentManager.setSteeringMode re-reads the mode from the live
   * provider session after writing it and broadcasts the refreshed state to
   * every connected session — not only this one — because the provider (Pi)
   * emits no event when the mode changes.
   */
  private async handleSetSteeringModeRequest(
    msg: Extract<SessionInboundMessage, { type: "set_steering_mode_request" }>,
  ): Promise<void> {
    const { agentId, mode, requestId } = msg;
    try {
      await this.agentManager.setSteeringMode(agentId, mode);
      this.emit({
  ...
  /** T38B0c: same contract as handleSetSteeringModeRequest, for the follow-up mode. */
  private async handleSetFollowUpModeRequest(
    msg: Extract<SessionInboundMessage, { type: "set_follow_up_mode_request" }>,
  ): Promise<void> {
    const { agentId, mode, requestId } = msg;
    try {
      await this.agentManager.setFollowUpMode(agentId, mode);
      this.emit({
  ...
  /**
   * T38B0c: reads both queue modes fresh off the live provider session (see
   * AgentManager.getQueueModes) rather than any value cached on this
   * connection, so a client that only just connected — or one that missed an
   * earlier broadcast — can always catch up on demand.
   */
  private async handleGetQueueModesRequest(
    msg: Extract<SessionInboundMessage, { type: "get_queue_modes_request" }>,
  ): Promise<void> {
    const { agentId, requestId } = msg;
    try {
      const { steeringMode, followUpMode } = await this.agentManager.getQueueModes(agentId);
      this.emit({
        type: "get_queue_modes_response",
        payload: { requestId, agentId, steeringMode, followUpMode, error: null },
      });
    } catch (error) {
      this.emit({
        type: "get_queue_modes_response",
  ...
  /**
   * T131: same contract as handleSetSteeringModeRequest, for auto-compaction.
   * AgentManager.setAutoCompaction re-reads the state from the live provider
   * session after writing it and broadcasts the refreshed state, because Pi
   * emits no event when auto-compaction changes.
   */
  private async handleSetAutoCompactionRequest(
    msg: Extract<SessionInboundMessage, { type: "set_auto_compaction_request" }>,
  ): Promise<void> {
    const { agentId, enabled, requestId } = msg;
    try {
      await this.agentManager.setAutoCompaction(agentId, enabled);
      this.emit({
  ...
  /**
   * T131: same contract as handleGetQueueModesRequest, for auto-compaction —
   * always reads fresh off the live provider session (see
   * AgentManager.getAutoCompaction), never a cached value.
   */
  private async handleGetAutoCompactionRequest(
    msg: Extract<SessionInboundMessage, { type: "get_auto_compaction_request" }>,
  ): Promise<void> {
    const { agentId, requestId } = msg;
    try {
      const enabled = await this.agentManager.getAutoCompaction(agentId);
      this.emit({
  ...
  /**
   * Handle push token deregistration (T61). See the doc comment on
   * `UnregisterPushTokenMessageSchema` (packages/protocol) for why this
   * exists: registration is purely additive, so a superseded token only
   * stops receiving once it is explicitly removed here. Deregistering a
  ...
  private async handleAgentPermissionResponse(
    ...
    try {
      await respondToAgentPermission({
        ...
        requestId,
        response,
        // T111: attribute this resolution to the connected client whose
        // agent_permission_response produced it, so every other connected
        // client's `RequestArbitrator` can tell who answered.
        answeredBy: { clientId: this.clientId },
        logger: this.sessionLogger,
      });
    } catch (error) {
    ...
    try {
      ...
      try {
        dispatchResult = await sendPromptToAgent({
          ...
          prompt,
          messageId: msg.messageId,
          // T38B0c: routes this one message as a steer or a follow-up
          // (T38B0a's per-message field), independent of the session's
          // steering/follow-up mode. Undefined when the client omits it,
          // which preserves today's behaviour exactly.
          runOptions: msg.streamingBehavior
