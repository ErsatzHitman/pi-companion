...
describe("PiCliRuntime", () => {
  ...
  test("forwards a per-message streamingBehavior choice to Pi's prompt command (T97)", async () => {
    const child = createPiChild();
    const pendingPrompt = capturePendingCommand(child, "prompt");
    const session = await createRuntime(child).startSession({ cwd: "/workspace/project" });

    const promptPromise = session.prompt("keep going but faster", undefined, "steer");
    ...
    expect(promptCommand).toMatchObject({
      type: "prompt",
      message: "keep going but faster",
      streamingBehavior: "steer",
    });
  });

  test("omits streamingBehavior from the wire when the caller does not choose one (T97)", async () => {
    const child = createPiChild();
    const pendingPrompt = capturePendingCommand(child, "prompt");
    const session = await createRuntime(child).startSession({ cwd: "/workspace/project" });

    const promptPromise = session.prompt("plain message, no routing choice");
    const promptCommand = await pendingPrompt;
    writePiResponse(child, promptCommand, { agentInvoked: true });
    await promptPromise;

    expect(promptCommand).not.toHaveProperty("streamingBehavior");
    expect(promptCommand).toEqual({
      id: expect.any(String),
      type: "prompt",
      message: "plain message, no routing choice",
    });
  });
