...
export async function resolvePiSessionsDir(
  options: PiSessionDescriptorOptions = {},
): Promise<string> {
...
// RPC-first helpers with FS/marker fallback. Gated behind probe in the caller.
export async function tryGetEntriesViaRpc(
  runtimeSession: PiRuntimeSession,
): Promise<unknown[] | null> {
  try {
    const entries = await runtimeSession.getEntries();
...
// T142: `tryGetTreeViaRpc` removed — it had zero production callers (only
// its own declaration), same audit finding as the `get_tree` RPC arm it
// wrapped. See the doc comment above the (removed) `get_tree` arm in
// `rpc-types.ts`'s `PiRpcCommand` union for the full decision record.
