...
import { z } from "zod";
import type { PermissionAnsweredBy, QueueMode } from "@picompanion/protocol/messages";
...
import { PiUiDecoder } from "./ui-bridge/decoder.js";
import { PiUiStateStore } from "./ui-bridge/state.js";
import { PiUiActionRouter } from "./ui-bridge/actions.js";
import type {
  PiAgentSessionEvent,
  PiAgentMessage,
  PiAssistantContent,
...
// One-time warnings for tier-2 drops (factory / unknown method) — module-scoped so we log once per daemon.
const piTier2UnknownMethodWarned = new Set<string>();
let piWidgetFactoryWarned = false;
...
/**
 * T143: reads `CompactionResult.details` defensively. Pi types `details` as
 * `unknown` (the interface's generic `T`, unparametrized on the event) —
 * only Pi's own doc comment on `CompactionDetails` says `{ readFiles,
 * modifiedFiles }` is what actually lands there, so this never assumes the
 * shape and drops anything that is not an array of strings rather than
 * throwing or forwarding garbage.
 */
function readPiCompactionFileLists(
  details: unknown,
  ...
  const readFiles = toStringArray(candidate.readFiles);
  const modifiedFiles = toStringArray(candidate.modifiedFiles);
  if (readFiles === undefined && modifiedFiles === undefined) {
    return undefined;
  }
  return {
...
function getAssistantMessageText(message: Extract<PiAgentMessage, { role: "assistant" }>): string {
  return message.content
    .filter((part): part is Extract<PiAssistantContent, { type: "text" }> => part.type === "text")
    .map((part) => part.text)
    .join("\n\n");
}
...
export class PiRpcAgentSession implements AgentSession {
  ...
  // Pi's agent_end is not turn-terminal: _runAgentPrompt loops through retry,
  // compaction, and queued-message continuations (each producing its own
  // agent_end) before emitting a single, final agent_settled from its
  // `finally` block. We buffer the latest agent_end's messages here and only
  // complete the turn on agent_settled so a continuation can't end the turn
  // early and orphan the rest of the run under an undefined turnId.
  private pendingAgentEndMessages: PiAgentMessage[] = [];
  private activeClientMessageId: string | null = null;
  private activeAssistantMessageId: string | null = null;
  private readonly assistantTextByResponseId = new Map<string, string>();
  ...
  getUiBridgeStateStore(): PiUiStateStore {
    return this.uiStateStore;
  }
  getUiBridgeActionRouter(): PiUiActionRouter {
    return this.uiActionRouter;
  }
  ...
  constructor(options: PiRpcAgentSessionOptions) {
    ...
    this.uiStateStore = new PiUiStateStore(
      (evt) => this.emit(evt),
      (agentId, reason) => {
        const payload = { v: 1, op: "resync", reason: reason ?? "agent-seq-gap" };
        const b64 = Buffer.from(JSON.stringify(payload)).toString("base64url");
        void this.runtimeSession.prompt(`/pi_ui_event ${b64}`).catch(() => {});
        this.emit({
          type: "pi_notice",
          provider: this.provider,
          level: "warning",
          message: `Pi UI bridge requested resync for ${agentId} (${reason ?? "agent-seq-gap"})`,
        });
      },
      (level, message) => {
        this.emit({ type: "pi_notice", provider: this.provider, level, message });
      },
    );
      ...
      onOp: (op) => {
        ...
        switch (op.kind) {
          case "set":
            ...
            break;
          case "patch":
            this.uiStateStore.applyPatch(agentId, op.id, op.ns, op.patch, op.agentSeq);
            break;
          case "remove":
            this.uiStateStore.applyRemove(agentId, op.id, op.ns, op.agentSeq);
            break;
          case "clear":
            this.uiStateStore.applyClear(agentId, op.agentSeq);
            break;
          case "sync":
            this.uiStateStore.applySync(agentId, op.elements, op.agentSeq);
            break;
          case "append":
            this.uiStateStore.applyAppend(agentId, op.id, op.ns, op.data, op.agentSeq);
            break;
          case "channel":
            this.uiStateStore.applyChannel(agentId, op.channel, op.payload, op.agentSeq);
            break;
        }
      },
  ...
  async startTurn(prompt: AgentPromptInput, options?: AgentRunOptions): Promise<StartTurnResult> {
    if (this.activeTurnId) {
      ...
      const existingTurnId = this.activeTurnId;
      // T107: an explicit per-message options.streamingBehavior overrides
      // this branch's own activeTurnStarted-derived default, exactly the way
      // the idle branch below already forwards it to override Pi's prompt()
      // routing. Omitting the field preserves today's activeTurnStarted-only
      ...
      void (async () => {
        try {
          ...
          this.emit({
            ...
            provider: this.provider,
            level: "error",
            message: `Failed to ${useSteer ? "steer" : "queue follow-up"} Pi turn: ${toDiagnosticErrorMessage(error)}`,
  ...
  async getRuntimeInfo(): Promise<AgentRuntimeInfo> {
    ...
    return {
      ...
      // T38B0c: carried in `extra` (not new AgentRuntimeInfo fields) so every
      // existing runtimeInfo broadcast — the same one AgentManager already
      // pushes to every connected client on any change — keeps a second
      // client's steering/follow-up mode view current for free, with no new
      // wire message needed.
      extra: {
        steeringMode: this.state.steeringMode ?? null,
        followUpMode: this.state.followUpMode ?? null,
      },
    };
  }
  ...
  // T38B0c: mirrors Pi's set_steering_mode / set_follow_up_mode RPC commands
  // (see PiQueueMode in rpc-types.ts). Pi emits NO event when either mode
  // changes — verified against the installed Pi's
  // dist/core/agent-session.js:1340-1341,1348,1356, a plain property set with
  // no emit — so refreshState() below is not a courtesy; it is the only way
  // this.state ever reflects the change at all, for this client or any
  // other.
  async setSteeringMode(mode: QueueMode): Promise<void> {
    await this.runtimeSession.request({ type: "set_steering_mode", mode });
    await this.refreshState();
  }

  async setFollowUpMode(mode: QueueMode): Promise<void> {
    await this.runtimeSession.request({ type: "set_follow_up_mode", mode });
    await this.refreshState();
  }

  // Re-reads both modes from the live Pi session rather than trusting
  // this.state's current value, for the same reason setSteeringMode /
  // setFollowUpMode refresh after writing: nothing pushes a change, so a
  // stale read here would silently show a mode nobody actually asked for
  // anymore (e.g. changed by a different connected client after this
  // session's last refresh).
  async getQueueModes(): Promise<{
  ...
  // T131: same "runtimeSession call, then refresh, since Pi emits no event"
  // shape as setSteeringMode/setFollowUpMode above.
  async setAutoCompaction(enabled: boolean): Promise<void> {
    await this.runtimeSession.setAutoCompaction(enabled);
    await this.refreshState();
  }

  // Re-reads from the live Pi session rather than trusting this.state's
  // current value, for the same reason getQueueModes does.
  ...
  private handleExtensionUiRequest(
    ...
    // PIUI bridge — must be first before PASEO checks; never leak marker to transcript
    {
      const raw =
        optionalString((event as Record<string, unknown>).message as string | undefined) ??
        optionalString(event.message);
      if (event.method === "notify" && raw?.startsWith(PIUI_MARKER)) {
        this.uiDecoder.ingest(raw);
        return;
      }
    }
    // Tier-2: setStatus — ephemeral status chip (undefined text = remove)
    if (
      event.method === "setStatus" &&
      typeof (event as Record<string, unknown>).key === "string" &&
      (event as Record<string, unknown>).key
    ) {
      const key = (event as Record<string, unknown>).key as string;
      const textRaw = (event as Record<string, unknown>).text;
    ...
    // Tier-2: setWidget — string-array only; factory without lines dropped with one-time warning
    if (
      ...
      const key = (event as Record<string, unknown>).key as string;
      const lines = (event as Record<string, unknown>).lines;
      const placementRaw = (event as Record<string, unknown>).placement;
      const placement = typeof placementRaw === "string" ? placementRaw : undefined;
      if (Array.isArray(lines) && (lines as unknown[]).every((v) => typeof v === "string")) {
        this.emit({
          type: "pi_widget",
          provider: this.provider,
          key,
        ...
        if (!piWidgetFactoryWarned) {
          piWidgetFactoryWarned = true;
          console.warn(
            `[pi] setWidget factory dropped for key=${key}: lines missing or not string[]`,
          );
        }
      }
      return;
    ...
    // Tier-2: setTitle — daemon-only; session lacks agentManager ref, emit pi_notice fallback
    if (
      event.method === "setTitle" &&
      typeof (event as Record<string, unknown>).title === "string"
    ) {
      const title = ((event as Record<string, unknown>).title as string).trim();
      if (title) {
        this.emit({
          type: "pi_notice",
          provider: this.provider,
    ...
    // Tier-2: set_editor_text / setEditorText — composer push
    if (
      (event.method === "set_editor_text" || event.method === "setEditorText") &&
    ...
    if (event.method === "notify" && message) {
      ...
      const levelRaw = optionalString((event as Record<string, unknown>).level);
    ...
    if (!request) {
      const knownDialog = new Set(["select", "input", "editor", "confirm"]);
      if (
        typeof event.method === "string" &&
        !knownDialog.has(event.method) &&
        !piTier2UnknownMethodWarned.has(event.method)
      ) {
  ...
  private handleSessionEvent(event: PiAgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "compaction_start":
        ...
        return;
      case "compaction_end": {
        // T143: `event.result` is Pi's own `CompactionResult` when Pi
        // supplies one (a failed/aborted compaction has none — see
        // `PiCompactionResult` in `./rpc-types.ts`). `details` is
        // typed `unknown` upstream, so `readPiCompactionFileLists` reads
        // it defensively rather than trusting the shape.
        const result = event.result;
        const fileLists = readPiCompactionFileLists(result?.details);
        this.emitCompactionTimeline({
          turnId,
          item: {
            type: "compaction",
            ...
            ...(result?.estimatedTokensAfter !== undefined
              ...
              : {}),
            ...(fileLists?.readFiles !== undefined ? { filesRead: fileLists.readFiles } : {}),
            ...(fileLists?.modifiedFiles !== undefined
              ? { filesModified: fileLists.modifiedFiles }
              : {}),
          },
        });
      ...
      case "agent_end":
        // Not turn-terminal - see pendingAgentEndMessages above. willRetry is
        // now purely informational (auto_retry_start/auto_retry_end already
        // report retry progress); it must never gate turn completion, since
        // Pi computes it false for the compaction and queued-message
        // continuations that also loop back through agent_end.
      ...
      case "agent_settled":
        ...
        return;
      case "turn_end":
        // Turn boundary for accurate sequencing; completeTurn already handles agent_end/settled.
        return;
      case "queue_update": {
        const steering = (event as { steering?: string[] }).steering ?? [];
        const followUp = (event as { followUp?: string[] }).followUp ?? [];
        this.emit({
      ...
      case "auto_retry_start":
        this.emit({
          ...
          error: (event as { errorMessage?: string }).errorMessage,
          turnId,
        });
        return;
      case "auto_retry_end": {
        const success = (event as { success: boolean }).success;
        const finalError = (event as { finalError?: string }).finalError;
        if (success === false && finalError) {
          this.emit({
            type: "pi_notice",
            provider: this.provider,
            level: "error",
            message: `Auto-retry failed after ${(event as { attempt: number }).attempt} attempts: ${finalError}`,
            turnId,
          });
        }
        return;
      ...
      case "summarization_retry_attempt_start": {
        ...
        const phase = source === "branchSummary" ? "branchSummary" : "compaction";
        this.emit({
          type: "pi_retry",
          provider: this.provider,
          phase,
      ...
      case "extension_error":
        this.emit({
          ...
          provider: this.provider,
          level: "error",
          message: (event as { error: string }).error,
          source: (event as { extensionPath?: string }).extensionPath,
          turnId,
        });
        return;
      default:
  ...
  private handleMessageUpdate(
    ...
    if (event.assistantMessageEvent.type === "text_delta") {
      ...
      if (key) {
        const prev = this.assistantTextByResponseId.get(key) ?? "";
        this.assistantTextByResponseId.set(key, prev + delta);
      }
      this.emit({
        type: "timeline",
        provider: this.provider,
  ...
  private handleMessageStart(event: Extract<PiAgentSessionEvent, { type: "message_start" }>): void {
    if (event.message.role === "assistant") {
      ...
      if (
        ...
        this.assistantTextByResponseId.set(this.activeAssistantMessageId, "");
      }
    }
  }
  ...
  probeStartupCapabilities(): void {
    void (async () => {
      try {
        await this.runtimeSession.getCommands();
      } catch (error) {
        this.emit({
          type: "pi_notice",
          provider: this.provider,
          level: "warning",
          message: `Pi capability probe: get_commands failed (${toDiagnosticErrorMessage(error)}). Pi binary may be outdated — steer/follow_up/get_entries unavailable.`,
        });
        return;
      }
      try {
        await this.runtimeSession.getSessionStats();
      } catch (error) {
        this.emit({
          type: "pi_notice",
          provider: this.provider,
          level: "warning",
          message: `Pi capability probe: get_session_stats failed (${toDiagnosticErrorMessage(error)}).`,
        });
      }
      try {
        await this.runtimeSession.getEntries();
      } catch (error) {
        const msg = toDiagnosticErrorMessage(error);
        if (/unknown command|get_entries/i.test(msg)) {
          this.emit({
            type: "pi_notice",
            provider: this.provider,
            level: "warning",
            message: `Pi capability probe: get_entries unavailable (${msg}). Update Pi for full tree/history support.`,
          });
        }
      }
    })();
...
export class PiRpcAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    try {
      ...
      session.probeStartupCapabilities();
      return session;
    } catch (error) {
