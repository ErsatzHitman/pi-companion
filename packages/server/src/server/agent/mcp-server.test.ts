...
import type {
  ...
  StoredSchedule,
  UpdateScheduleInput,
} from "@picompanion/protocol/schedule/types";
...
function configureOpenCodeProviderStub(
  ...
  stub.hasProvider.mockImplementation((provider: AgentProvider) =>
    Object.prototype.hasOwnProperty.call(modesByProvider, provider),
  );
  stub.getProviderLabel.mockImplementation((provider: AgentProvider) => {
    const entry = entries.find((e) => e.provider === provider);
    return entry?.label ?? provider;
  });
  stub.listProviders.mockImplementation(async (input: unknown) => {
  ...
  stub.getProvider.mockImplementation(async (input: unknown) => {
  ...
  stub.listModes.mockImplementation(async (input: unknown) => {
    const opts = input as { provider: AgentProvider };
    return modesByProvider[opts.provider] ?? [];
  });
  stub.resolveCreateConfig.mockImplementation(async (input: unknown) => {
    const opts = input as {
      provider: AgentProvider;
...
function createClaudeOnlyManager(): ProviderSnapshotManagerStub["manager"] {
  ...
  stub.hasProvider.mockImplementation((provider: AgentProvider) => provider === "claude");
  return stub.manager;
}
