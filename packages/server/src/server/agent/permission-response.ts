...
export interface RespondToAgentPermissionParams {
  ...
  requestId: string;
  response: AgentPermissionResponse;
  /** The connected client answering, when the caller knows it (T111). */
  answeredBy?: PermissionAnsweredBy;
  logger: Logger;
}
...
export async function respondToAgentPermission(
  ...
  const { agentManager, agentId, requestId, response, answeredBy, logger } = params;
  logger.debug(
    { agentId, requestId },
    `Handling permission response for agent ${agentId}, request ${requestId}`,
  );

  const result = await agentManager.respondToPermission(agentId, requestId, response, answeredBy);
