...
// Pi is the only built-in provider in this repository, and its manifest entry
// declares `defaultModeId: null`. The Paseo reference manifest still carried the
// providers these fakes emulate, so AgentManager could read their default mode
// from `getAgentProviderDefinition`. The fakes now supply the same defaults
// through the `AgentClient.resolveDefaultModeId` hook that AgentManager consults
// before falling back to the manifest, which keeps the assertions unchanged.
const TEST_PROVIDER_DEFAULT_MODE_IDS: Record<string, string> = {
  codex: "auto-review",
};
...
class TestAgentClient implements AgentClient {
  ...
  async resolveDefaultModeId(_input: ResolveAgentDefaultModeInput): Promise<string | undefined> {
    return TEST_PROVIDER_DEFAULT_MODE_IDS[this.provider];
  }
...
// T38B0c: queue-mode (steering / follow-up) changes must reach the live
// provider session and, because at least the Pi provider emits no event
// when a mode changes, the daemon must actively re-read state and broadcast
// it — never trust a value cached before the change.
class QueueModeTestAgentSession extends TestAgentSession {
  ...
  override async getRuntimeInfo() {
    ...
    return {
      ...base,
      extra: { steeringMode: this.steeringMode, followUpMode: this.followUpMode },
    };
  }

  // Mirrors the real Pi provider's silence: a plain property set, no event
  // pushed to any subscriber. Only a fresh getRuntimeInfo()/getQueueModes()
  // read (below) ever observes the change.
  ...
  async getQueueModes() {
    return { steeringMode: this.steeringMode, followUpMode: this.followUpMode };
  }
}

class QueueModeTestAgentClient extends TestAgentClient {
  override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
    return new QueueModeTestAgentSession(config);
  }
}

test("setSteeringMode re-reads live state and broadcasts it to every subscriber", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-queue-mode-"));
  const manager = new AgentManager({
    clients: { codex: new QueueModeTestAgentClient() },
    logger,
  ...
  // Two independently subscribed "clients" — neither polls; both must see
  // the change purely from the broadcast setSteeringMode triggers.
...
test("setFollowUpMode re-reads live state and broadcasts it to every subscriber", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-queue-mode-"));
  const manager = new AgentManager({
    clients: { codex: new QueueModeTestAgentClient() },
    logger,
  ...
  await manager.setFollowUpMode(snapshot.id, "all");
...
test("getQueueModes reads the live session rather than a value cached from before an out-of-band change", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-queue-mode-"));
  const manager = new AgentManager({
    clients: { codex: new QueueModeTestAgentClient() },
    logger,
  ...
  const before = await manager.getQueueModes(snapshot.id);
  expect(before).toEqual({ steeringMode: "one-at-a-time", followUpMode: "one-at-a-time" });

  // Mutate the underlying session directly (as Pi's real silent property set
  // would), completely bypassing manager.setSteeringMode/setFollowUpMode.
  const agent = manager.getAgent(snapshot.id);
  const session = agent?.session as unknown as QueueModeTestAgentSession;
  session.steeringMode = "all";

  const after = await manager.getQueueModes(snapshot.id);
  expect(after).toEqual({ steeringMode: "all", followUpMode: "one-at-a-time" });
});

test("setSteeringMode throws for a provider session that does not support it", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-queue-mode-"));
  const manager = new AgentManager({
    clients: { codex: new TestAgentClient() },
    logger,
  ...
  await expect(manager.setSteeringMode(snapshot.id, "all")).rejects.toThrow(
    "Agent session does not support a steering mode",
  );
});

test("getQueueModes returns nulls for a provider session that does not support queue modes", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-queue-mode-"));
  const manager = new AgentManager({
    clients: { codex: new TestAgentClient() },
    logger,
  ...
  await expect(manager.getQueueModes(snapshot.id)).resolves.toEqual({
    steeringMode: null,
    followUpMode: null,
  });
});

// T131: auto-compaction changes must reach the live provider session and,
// same as queue modes above, the daemon must re-read and broadcast state
// rather than trust a value cached before the change.
class AutoCompactionTestAgentSession extends TestAgentSession {
  ...
  override async getRuntimeInfo() {
    ...
    return {
      ...base,
      extra: { autoCompactionEnabled: this.autoCompactionEnabled },
    };
  }
  ...
  async getAutoCompaction(): Promise<boolean> {
    return this.autoCompactionEnabled;
  }
}

class AutoCompactionTestAgentClient extends TestAgentClient {
  override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
    return new AutoCompactionTestAgentSession(config);
  }
}

test("setAutoCompaction re-reads live state and broadcasts it to every subscriber", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-auto-compaction-"));
  const manager = new AgentManager({
    clients: { codex: new AutoCompactionTestAgentClient() },
    logger,
  ...
  await manager.setAutoCompaction(snapshot.id, false);

  for (const events of [firstClientEvents, secondClientEvents]) {
    const last = events.at(-1);
    expect(last?.type).toBe("agent_state");
    if (last?.type === "agent_state") {
      expect(last.agent.runtimeInfo?.extra).toEqual({ autoCompactionEnabled: false });
    }
  }
});

test("getAutoCompaction reads the live session rather than a value cached from before an out-of-band change", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-auto-compaction-"));
  const manager = new AgentManager({
    clients: { codex: new AutoCompactionTestAgentClient() },
    logger,
  ...
  const before = await manager.getAutoCompaction(snapshot.id);
  expect(before).toBe(true);

  const agent = manager.getAgent(snapshot.id);
  const session = agent?.session as unknown as AutoCompactionTestAgentSession;
  session.autoCompactionEnabled = false;

  const after = await manager.getAutoCompaction(snapshot.id);
  expect(after).toBe(false);
});

test("setAutoCompaction throws for a provider session that does not support it", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-auto-compaction-"));
  const manager = new AgentManager({
    clients: { codex: new TestAgentClient() },
    logger,
  ...
  await expect(manager.setAutoCompaction(snapshot.id, false)).rejects.toThrow(
    "Agent session does not support auto-compaction",
  );
});

test("getAutoCompaction returns null for a provider session that does not support it", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-auto-compaction-"));
  const manager = new AgentManager({
    clients: { codex: new TestAgentClient() },
    logger,
