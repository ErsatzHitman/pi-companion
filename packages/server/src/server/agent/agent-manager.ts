...
} from "@picompanion/protocol/agent-lifecycle";
import {
  getParentAgentIdFromLabels,
  isDelegatedAgent,
  PARENT_AGENT_ID_LABEL,
} from "@picompanion/protocol/agent-labels";
...
export class AgentManager {
  ...
  // T38B0c: the provider (currently only Pi) emits no event when either mode
  // changes, so getRuntimeInfo() below is re-read from the live session, not
  // taken from any cached value, and this.emitState(agent) unconditionally
  // broadcasts the refreshed runtimeInfo to every connected session (see
  // AgentManager.subscribe/dispatch) — not only the caller of this method.
  // That broadcast is what keeps a second connected client from going stale.
  async setSteeringMode(agentId: string, mode: QueueMode): Promise<void> {
    const agent = this.requireSessionAgent(agentId);

    if (!agent.session.setSteeringMode) {
      throw new Error("Agent session does not support a steering mode");
    }

    await agent.session.setSteeringMode(mode);
    await this.drainSessionEvents(agentId);
  ...
  async setFollowUpMode(agentId: string, mode: QueueMode): Promise<void> {
    const agent = this.requireSessionAgent(agentId);

    if (!agent.session.setFollowUpMode) {
      throw new Error("Agent session does not support a follow-up mode");
    }

    await agent.session.setFollowUpMode(mode);
    await this.drainSessionEvents(agentId);
    agent.runtimeInfo = await agent.session.getRuntimeInfo();
    this.touchUpdatedAt(agent);
    this.emitState(agent);
  ...
  /**
   * Reads both queue modes fresh off the live session (never a cached
   * value — see setSteeringMode's note on why) and, since that read may
   * itself correct a runtimeInfo.extra the daemon had not caught up on yet,
   * broadcasts the refreshed runtimeInfo the same way a set does.
   */
  async getQueueModes(
    agentId: string,
    ...
    if (!agent.session.getQueueModes) {
      return { steeringMode: null, followUpMode: null };
    }

    const modes = await agent.session.getQueueModes();
    agent.runtimeInfo = await agent.session.getRuntimeInfo();
    this.touchUpdatedAt(agent);
    this.emitState(agent);
    return modes;
  ...
  // T131: same "provider emits no event, so re-read and broadcast" shape as
  // setSteeringMode above.
  async setAutoCompaction(agentId: string, enabled: boolean): Promise<void> {
    const agent = this.requireSessionAgent(agentId);

    if (!agent.session.setAutoCompaction) {
      throw new Error("Agent session does not support auto-compaction");
    }

    await agent.session.setAutoCompaction(enabled);
    await this.drainSessionEvents(agentId);
    agent.runtimeInfo = await agent.session.getRuntimeInfo();
    this.touchUpdatedAt(agent);
    this.emitState(agent);
  ...
  /**
   * Reads auto-compaction fresh off the live session (never cached — same
   * reason getQueueModes does), broadcasting the refreshed runtimeInfo the
   * same way a set does.
   */
  async getAutoCompaction(agentId: string): Promise<boolean | null> {
    const agent = this.requireSessionAgent(agentId);

    if (!agent.session.getAutoCompaction) {
      return null;
    }

    const enabled = await agent.session.getAutoCompaction();
    agent.runtimeInfo = await agent.session.getRuntimeInfo();
    this.touchUpdatedAt(agent);
    this.emitState(agent);
    return enabled;
  ...
  async respondToPermission(
    ...
    try {
      const result = await agent.session.respondToPermission(requestId, response, answeredBy);
