...
import { PiSessionWatcher } from "./agent/providers/pi/pi-session-watcher.js";
import { PiLiveTailWatcher } from "./agent/providers/pi/pi-live-tail.js";
import { resolveDaemonVersion } from "./daemon-version.js";
import type { AgentClient, AgentProvider } from "./agent/agent-sdk-types.js";
import type { FirstAgentContext, TerminalProfile } from "@picompanion/protocol/messages";
import type {
...
export async function createPaseoDaemon(
  ...
  let piSessionWatcher: PiSessionWatcher | null = null;
  let piLiveTailWatcher: PiLiveTailWatcher | null = null;
  ...
  const allowedOrigins = new Set([
    ...
    // Packaged desktop renderers use the custom picompanion:// protocol scheme.
    "picompanion://app",
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            // Pi auto-discovery + live tail: watch ~/.pi/agent/sessions for CLI/Herdr sessions
            // Debounce 500ms, poll backstop 30s, auto-import live (<5min) sessions so they appear without Import click
            try {
              piSessionWatcher = new PiSessionWatcher({
                logger,
                agentManager,
                agentStorage,
                workspaceProvisioning,
                broadcast: (msg: unknown) => {
                  try {
                    // Broadcast to all trusted clients; cast to allow recent_provider_sessions_changed
                    (wsServer as unknown as { broadcast: (m: unknown) => void })?.broadcast?.(
                      msg as never,
                    );
                  } catch {}
                },
              });
              await piSessionWatcher.start();
            } catch (error) {
              logger.warn({ err: error }, "Failed to start Pi session watcher");
            }
            try {
              piLiveTailWatcher = new PiLiveTailWatcher({ logger, agentManager });
              piLiveTailWatcher.start();
            } catch (error) {
              logger.warn({ err: error }, "Failed to start Pi live tail watcher");
            }
          };
  ...
  const stop = async () => {
    try {
      piSessionWatcher?.stop();
    } catch {}
    try {
      piLiveTailWatcher?.stop();
    } catch {}
