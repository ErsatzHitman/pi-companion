...
describe("DaemonConfigStore", () => {
  ...
  test("patch removes provider entries from config.json", () => {
    ...
    expect(next.providers.pi).toEqual({ enabled: false });
    const persisted = loadPersistedConfig(paseoHome);
    expect(persisted.agents?.providers?.gemini).toBeUndefined();
    expect(persisted.agents?.providers?.pi).toEqual({ enabled: false });
  });
  ...
  test("patch removes deleted providers from metadata generation", () => {
    ...
    expect(next.metadataGeneration.providers).toEqual([{ provider: "pi", model: "haiku" }]);
    const persisted = loadPersistedConfig(paseoHome);
    expect(persisted.agents?.metadataGeneration).toEqual({
      providers: [{ provider: "pi", model: "haiku" }],
    });
  });
  ...
  test("patch persists provider additional models into config.json", () => {
    ...
    const persisted = loadPersistedConfig(paseoHome);
    expect(persisted.agents?.providers?.pi).toEqual({
      additionalModels: [
        {
          id: "pi-custom",
          label: "pi-custom",
  ...
  test("patch persists clearing metadata generation providers into config.json", () => {
    ...
    const store = new DaemonConfigStore(
      paseoHome,
      {
        ...
        enableTerminalAgentHooks: false,
        appendSystemPrompt: "",
        metadataGeneration: { providers: [{ provider: "pi", model: "haiku" }] },
      },
      undefined,
    );
