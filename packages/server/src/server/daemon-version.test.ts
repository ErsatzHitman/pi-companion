...
describe("resolveDaemonVersion", () => {
  it("resolves server version by walking up to @picompanion/server package.json", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
      JSON.stringify({ name: "@picompanion/server", version: "9.8.7" }),
      "utf8",
    );
  ...
  it("throws when @picompanion/server package metadata cannot be resolved", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
  ...
  it("throws when @picompanion/server version is missing", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
      JSON.stringify({ name: "@picompanion/server" }),
      "utf8",
    );
    const nestedDir = path.join(root, "dist", "server");
    mkdirSync(nestedDir, { recursive: true });
