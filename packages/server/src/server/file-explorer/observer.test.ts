...
describe("FileObserver", () => {
  ...
  test("publishes shared watcher updates in each subscriber's path coordinates", async (ctx) => {
    const root = await workspace();
    ...
    try {
      ...
      // Windows refuses directory symlinks without elevation/developer mode.
      const code = (error as NodeJS.ErrnoException).code;
      if (code === "EPERM" || code === "EACCES" || code === "ENOSYS") {
        ctx.skip(`symlink creation unavailable on this platform (${code})`);
        return;
      }
      throw error;
    }
