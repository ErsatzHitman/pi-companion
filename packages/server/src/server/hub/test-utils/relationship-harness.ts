...
export class HubRelationshipHarness {
  ...
  private readonly pi: ControlledAgentClient;

  private constructor(private readonly mcpEnabled: boolean) {
    this.pi = new ControlledAgentClient(
      new FakeAgentClient("pi", {
  ...
  async agentCreationAttempts(count: number): Promise<void> {
    await this.pi.creationAt(count);
  }

  finishAgentCreation(): void {
    this.pi.finishCreation();
  }
  ...
  providerCreations(): number {
    return this.pi.creations;
  }

  providerResumes(): number {
    return this.pi.resumes;
  }
  ...
  latestProviderCreateConfig(): AgentSessionConfig | null {
    return this.pi.createdConfigs.at(-1) ?? null;
  }
  ...
  latestCreatedCwd(): string | null {
    return this.pi.createdConfigs.at(-1)?.cwd ?? null;
  }
  ...
  async reconstructAndReplay(executionId = "execution-1") {
    ...
    const manager = new AgentManager({
      clients: { ...createTestAgentClients(), pi: new FakeAgentClient("pi", {}) },
      registry: storage,
