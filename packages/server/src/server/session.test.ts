...
import type { WorkspaceDescriptorPayload } from "@picompanion/protocol/messages";
import {
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file.upload.cancel.request reaches the daemon and confirms a real discard (T163)", async () => {
    const paseoHome = makeDir("file-access-upload-cancel-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, paseoHome });
    ...
    // Reaching this handler through `session.handleMessage` — the same
    // entry point a real inbound WebSocket message is dispatched through
    // — is what proves the wire-level route exists, not just the
    // subsystem method it eventually calls.
    await session.handleMessage({
      type: "file.upload.cancel.request",
      uploadRequestId: "req-upload-cancel",
      requestId: "req-cancel-rpc",
    });

    const response = messages.find((message) => message.type === "file.upload.cancel.response");
    if (response?.type !== "file.upload.cancel.response") {
    ...
    // A FileEnd delivered after the confirmed cancel must not complete the
    // upload — there is nothing left pending for it to complete.
    await session.handleBinaryFrame({
      kind: "file_transfer",
      frame: uploadFrame({ opcode: FileTransferOpcode.FileEnd, requestId: "req-upload-cancel" }),
    });
    expect(messages.some((message) => message.type === "file.upload.response")).toBe(false);
  });
});
...
describe("T38B0c: queue-mode requests and per-message routing", () => {
  function liveAgentManager(overrides: { [K in keyof SessionOptions["agentManager"]]?: unknown }): {
    [K in keyof SessionOptions["agentManager"]]?: unknown;
  } {
    return {
  ...
  test("set_steering_mode_request: success emits an accepted response", async () => {
    const messages: SessionOutboundMessage[] = [];
    const setSteeringMode = vi.fn().mockResolvedValue(undefined);
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({ setSteeringMode }),
    });
    ...
    await session.handleMessage({
      ...
      agentId: "agent-1",
      mode: "all",
      requestId: "req-steer-ok",
    });

    expect(setSteeringMode).toHaveBeenCalledWith("agent-1", "all");
    expect(messages).toEqual([
      {
        type: "set_steering_mode_response",
        payload: { requestId: "req-steer-ok", agentId: "agent-1", accepted: true, error: null },
      },
    ]);
  });

  test("set_steering_mode_request: failure emits a rejected response with the error message", async () => {
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({
        setSteeringMode: vi.fn().mockRejectedValue(new Error("steering boom")),
      }),
    });
  ...
  test("set_follow_up_mode_request: success emits an accepted response", async () => {
    const messages: SessionOutboundMessage[] = [];
    const setFollowUpMode = vi.fn().mockResolvedValue(undefined);
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({ setFollowUpMode }),
    });
    ...
    await session.handleMessage({
      ...
      agentId: "agent-1",
      mode: "one-at-a-time",
      requestId: "req-followup-ok",
    });

    expect(setFollowUpMode).toHaveBeenCalledWith("agent-1", "one-at-a-time");
    expect(messages).toEqual([
      {
  ...
  test("set_follow_up_mode_request: failure emits a rejected response with the error message", async () => {
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({
        setFollowUpMode: vi.fn().mockRejectedValue(new Error("follow-up boom")),
      }),
    });
    ...
    expect(messages).toEqual([
      {
        ...
        payload: {
          ...
          agentId: "agent-1",
          accepted: false,
          error: "follow-up boom",
        },
      },
    ]);
  });

  test("get_queue_modes_request: reads both modes fresh, not from a value cached on this connection", async () => {
    const messages: SessionOutboundMessage[] = [];
    const getQueueModes = vi
      .fn()
      .mockResolvedValue({ steeringMode: "all", followUpMode: "one-at-a-time" });
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({ getQueueModes }),
    });
    ...
    expect(getQueueModes).toHaveBeenCalledWith("agent-1");
    expect(messages).toEqual([
      {
  ...
  test("get_queue_modes_request: failure emits null modes with the error message", async () => {
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({
        getQueueModes: vi.fn().mockRejectedValue(new Error("get boom")),
      }),
    });
  ...
  // Contract fixture: drives BOTH per-message routings (steer / followUp)
  // and confirms the field reaches AgentManager's out-of-band dispatch path
  // (tryRunOutOfBand) exactly as the client sent it — and that omitting the
  // field preserves today's behaviour (no streamingBehavior key at all).
  test.each(["steer", "followUp"] as const)(
    "send_agent_message_request: routes streamingBehavior=%s through to the agent run's runOptions",
    async (streamingBehavior) => {
      const messages: SessionOutboundMessage[] = [];
      const tryRunOutOfBand = vi.fn(() => true);
      const session = createSessionForTest({
        messages,
        agentManager: liveAgentManager({ tryRunOutOfBand }),
      ...
      expect(messages).toEqual([
        {
          ...
          payload: { requestId: "req-send-ok", agentId: "agent-1", accepted: true, error: null },
        },
      ]);
    },
  );

  test("send_agent_message_request: omitting streamingBehavior sends undefined runOptions.streamingBehavior", async () => {
    const messages: SessionOutboundMessage[] = [];
    const tryRunOutOfBand = vi.fn(() => true);
    const session = createSessionForTest({
      messages,
    ...
    await session.handleMessage({
      ...
      agentId: "agent-1",
      text: "hello",
      requestId: "req-send-no-behavior",
    });
...
describe("T131: auto-compaction requests", () => {
  ...
  test("set_auto_compaction_request: success emits an accepted response", async () => {
    const messages: SessionOutboundMessage[] = [];
    const setAutoCompaction = vi.fn().mockResolvedValue(undefined);
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({ setAutoCompaction }),
    });
    ...
    await session.handleMessage({
      ...
      agentId: "agent-1",
      enabled: false,
      requestId: "req-compact-ok",
    });

    expect(setAutoCompaction).toHaveBeenCalledWith("agent-1", false);
    expect(messages).toEqual([
      {
        type: "set_auto_compaction_response",
        payload: { requestId: "req-compact-ok", agentId: "agent-1", accepted: true, error: null },
      },
    ]);
  });

  test("set_auto_compaction_request: failure emits a rejected response with the error message", async () => {
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({
        setAutoCompaction: vi.fn().mockRejectedValue(new Error("compaction boom")),
      }),
    });
    ...
    expect(messages).toEqual([
      {
        ...
        payload: {
          ...
          agentId: "agent-1",
          accepted: false,
          error: "compaction boom",
        },
      },
    ]);
  });

  test("get_auto_compaction_request: reads the value fresh, not from a value cached on this connection", async () => {
    const messages: SessionOutboundMessage[] = [];
    const getAutoCompaction = vi.fn().mockResolvedValue(true);
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({ getAutoCompaction }),
    });

    await session.handleMessage({
      type: "get_auto_compaction_request",
      agentId: "agent-1",
      requestId: "req-get-compact-ok",
    });

    expect(getAutoCompaction).toHaveBeenCalledWith("agent-1");
    expect(messages).toEqual([
      {
  ...
  test("get_auto_compaction_request: failure emits a null value with the error message", async () => {
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({
      messages,
      agentManager: liveAgentManager({
        getAutoCompaction: vi.fn().mockRejectedValue(new Error("get compact boom")),
      }),
    });
...
describe("agent_permission_resolved carries answeredBy (T111)", () => {
  test("attributes an outgoing agent_permission_response to this connection's clientId", async () => {
    const messages: SessionOutboundMessage[] = [];
    ...
    // `createSessionForTest` connects as clientId "test-client" — see its
    // `sessionOptions` above. `Session` must thread that connection's own
    // identity through as the answering client, not synthesize one at the
    // broadcast site.
  ...
  test("forwards a resolution's answeredBy from the real arbitrator onto the broadcast agent_permission_resolved", async () => {
    const messages: SessionOutboundMessage[] = [];
    const agentEventListeners: Array<(event: AgentManagerEvent) => void> = [];
    const session = createSessionForTest({
      messages,
    ...
    // `permission_resolved` is also forwarded generically as an
    // `agent_stream` message (`forwardAgentStream`) alongside the specific
    // `agent_permission_resolved` this test is about — filter to the
    // latter, mirroring `session.test.ts`'s own precedent above
    // (`messages.filter((message) => message.type === "agent_stream")`).
    const resolvedMessages = messages.filter(
      (message) => message.type === "agent_permission_resolved",
    );
  ...
  test("omits answeredBy from the broadcast when the resolution carries none (pre-T111 behavior)", async () => {
    const messages: SessionOutboundMessage[] = [];
    const agentEventListeners: Array<(event: AgentManagerEvent) => void> = [];
    const session = createSessionForTest({
      messages,
    ...
    const [broadcast] = resolvedMessages;
    const payload = broadcast && "payload" in broadcast ? (broadcast.payload as object) : {};
    expect(Object.prototype.hasOwnProperty.call(payload, "answeredBy")).toBe(false);
  });
});
...
describe("pi.ui.action.response / pi_ui_action_result carry answeredBy (T128)", () => {
  test("attributes an outgoing pi.ui.action.response to this connection's clientId", async () => {
    const messages: SessionOutboundMessage[] = [];
    // `getAgent` returning null drives `dispatchPiUiMessage`'s real catch
    // branch (`Agent ${agentId} not found`) — the same code path that emits
    // `pi.ui.action.response` on success, without needing to fake the full
    // sendPromptToAgent -> agentStorage/agentManager orchestration chain.
    const session = createSessionForTest({
      messages,
    ...
    // `dispatchPiUiMessage` dispatches its work in a fire-and-forget IIFE
    // (`void (async () => { ... })()`), so give the microtask queue a turn.
    await new Promise((resolve) => setTimeout(resolve, 0));

    const responses = messages.filter((message) => message.type === "pi.ui.action.response");
    expect(responses).toEqual([
      {
        ...
        payload: {
          ...
          // `createSessionForTest` connects as clientId "test-client" — see
          // its `sessionOptions` above. `Session` must thread that
          // connection's own identity through as the answering client, not
          // synthesize one at the broadcast site.
          answeredBy: { clientId: "test-client" },
        },
      },
    ]);
  });

  test("forwards a pi_ui_action_result's answeredBy through the generic agent_stream broadcast", async () => {
    const messages: SessionOutboundMessage[] = [];
    const agentEventListeners: Array<(event: AgentManagerEvent) => void> = [];
    const session = createSessionForTest({
      messages,
    ...
    // `pi_ui_action_result` has no bespoke handler in `subscribeToAgentEvents`
    // (unlike `permission_resolved`) — it only ever reaches a client through
    // the generic `forwardAgentStream` path, so this proves that pass-through
    // does not strip the new optional field.
    forward({
      type: "agent_stream",
      agentId: "agent-1",
    ...
    const streamMessages = messages.filter((message) => message.type === "agent_stream");
  ...
  test("omits answeredBy from the pi_ui_action_result broadcast when the event carries none (pre-T128 behavior)", async () => {
    const messages: SessionOutboundMessage[] = [];
    const agentEventListeners: Array<(event: AgentManagerEvent) => void> = [];
    const session = createSessionForTest({
      messages,
    ...
    const [broadcast] = streamMessages;
    const payload =
