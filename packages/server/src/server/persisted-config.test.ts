...
describe("PersistedConfigSchema agent provider runtime settings", () => {
  ...
  test("accepts provider command replace argv", () => {
      ...
      agents: {
        providers: {
          pi: {
            command: {
              mode: "replace",
              argv: ["docker", "run", "--rm", "my-pi-wrapper"],
            },
          },
        },
      },
    ...
    expect(parsed.agents?.providers?.pi?.command).toEqual([
      "docker",
      "run",
      "--rm",
      "my-pi-wrapper",
...
describe("provider overrides (new format)", () => {
  test("override built-in provider with command and env", () => {
    ...
    expect(parsed.agents?.providers?.pi).toEqual({
      command: ["/opt/custom/pi"],
      env: {
        PI_API_KEY: "sk-test",
      },
  ...
  test("new provider extending pi with label", () => {
    const parsed = PersistedConfigSchema.parse({
      agents: {
        providers: {
          zai: {
  ...
  test("enabled: false accepted", () => {
    ...
    expect(parsed.agents?.providers?.pi?.enabled).toBe(false);
  });
  ...
  test("order field accepted", () => {
    ...
    expect(parsed.agents?.providers?.pi?.order).toBe(1);
  });
