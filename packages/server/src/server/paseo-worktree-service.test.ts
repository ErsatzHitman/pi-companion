...
test("creates a worktree workspace at the selected project subdirectory", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const sourceDir = path.join(repoDir, "packages", "core");
  ...
  expect(result.workspace).toMatchObject({
    projectId: project.projectId,
    cwd: path.join(result.worktree.worktreePath, "packages", "core"),
...
test("maps a nested cwd from an existing Paseo worktree into the next worktree", async () => {
  ...
  cleanupPaths.push(tempDir);
  const paseoHome = path.join(tempDir, ".paseo");
  const projectDir = path.join(repoDir, "packages", "core");
  ...
  const sourceCwd = path.join(source.worktree.worktreePath, "packages", "core");
  ...
  expect(created.workspace.cwd).toBe(path.join(created.worktree.worktreePath, "packages", "core"));
