...
let wranglerCliPath: string | null = null;
try {
  wranglerCliPath = createRequire(import.meta.url).resolve("wrangler/bin/wrangler.js");
} catch {
  wranglerCliPath = null;
}
const shouldRunRelayE2e =
  (process.env.FORCE_RELAY_E2E === "1" ||
    Number((process.versions.node ?? "0").split(".")[0] ?? "0") < 25) &&
  wranglerCliPath !== null;
