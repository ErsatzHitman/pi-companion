...
import { loadConfig, resolvePaseoHome, spawnProcess } from "@picompanion/server";
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@picompanion/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@picompanion/server");
  ...
  throw new Error("Unable to resolve @picompanion/server package root for daemon runner");
}
