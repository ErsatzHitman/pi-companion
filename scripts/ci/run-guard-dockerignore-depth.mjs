 ...
 * than the repository root, right now? This is additive, strongest-possible
 * evidence when a
 * working tree happens to already carry build output (as T177's own audit
 * found: 6 nested `node_modules`, 10 nested `dist`, 2 nested
 * `*.tsbuildinfo`) — it is NOT required for this guard's correctness (the
 * structural family evidence in guard-dockerignore-depth.mjs covers the
 * seven patterns T177 fixed without it), and it is the ONLY evidence source
 * for `*.log`, which has no single owning tool config file (see that file's
 * "DISCLOSED GAP" comment). On a pristine checkout (this guard's own CI job:
 * a bare `actions/checkout`, no build, no install) this returns an empty
 * set, same as every other build-state-dependent guard in this directory.
 *
 ...
 * @returns {Set<string>}
 */
function findRealNestedOccurrences(root, bareNames) {
  ...
  function walk(dir, depth) {
    ...
    for (const entry of entries) {
      ...
      if (matched) {
        ...
        if (!isRoot) found.add(matched.name);
        continue;
      }
...
function main() {
  ...
  const diskNestedNames = findRealNestedOccurrences(root, bareNames);
