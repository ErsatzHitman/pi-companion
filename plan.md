...
The pinned slot above the composer is where a **docked widget** lives, not only where subagent state does: the session's todo list occupies it (T385), which is what the reference's own `todo` frame means by "a widget docked above the prompt bar, not a transcript block". An extension element that draws **in the transcript** is a third destination on the same screen, selected by its `placement` (`inline`) beside the pinned and sheet ones (T387); the reference's advisor, delegate-monitor and peer frames are that destination's drawing, and their `[ns]` header, `extension-bg` block, aligned key/value lines and severity glyphs are the treatment it carries. None of them is a separate route: `docs/ui-reference/pi-companion-app.html`'s five extension frames are a picture of _where each extension draws_, and the screen list above is what ships.
